...
function renderTotalDashboard(p) {
  ...
  const pendingPlans  = MOCK_BO_PLANS.filter(pl => pl.status === 'pending_total' && pl.group === p.budgetGroup).length;
  const pendingApps   = MOCK_BO_APPLICATIONS.filter(a => a.status === 'pending_total' && a.group === p.budgetGroup).length;
...
<div class="bo-fade">
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:24px">
    ${[
      { label:'총 예산', val: boFmt(grandTotal)+'원', sub:'전사 배정 총액', icon:'🏦', color:'#002C5F', bg:'#EFF6FF' },
      { label:'실차감 금액', val: boFmt(grandDeducted)+'원', sub:`소진율 ${deductedPct}%`, icon:'📉', color:'#1D4ED8', bg:'#EFF6FF' },
      { label:'가점유 금액', val: boFmt(grandHolding)+'원', sub:`홀딩률 ${holdingPct}%`, icon:'⏳', color:'#B45309', bg:'#FFF7ED' },
      { label:'사용 가능 잔액', val: boFmt(grandAvail)+'원', sub:'즉시 집행 가능', icon:'✅', color:'#059669', bg:'#F0FDF4' },
    ].map(k=>`
  ...
  <div style="display:grid;grid-template-columns:1fr 340px;gap:20px">
    <div class="bo-card" style="overflow:hidden">
      ...
      <table class="bo-table">
        ...
        <tbody>
          ${orgs.map(o=>{
            const pct = ((o.budget.deducted+o.budget.holding)/o.budget.total*100).toFixed(0);
            return `<tr>
    ...
    <div class="bo-card" style="padding:20px">
      ...
      <div style="display:flex;flex-direction:column;gap:12px">
        ${[
          { label:'교육계획 승인 대기', count: pendingPlans, color:'#1D4ED8', icon:'📋', menu:'plan-mgmt' },
          { label:'교육신청 승인 대기', count: pendingApps,  color:'#B45309', icon:'✅', menu:'approval' },
          { label:'결과보고 승인 대기', count: MOCK_BO_APPLICATIONS.filter(a=>a.type==='결과보고'&&a.status.startsWith('pending')&&a.group===p.budgetGroup).length, color:'#059669', icon:'📤', menu:'approval' },
        ].map(item=>`
...
<div class="bo-fade">
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:24px">
    ${[
      { label:'총 배정액', val:boFmt(org.budget.total)+'원', icon:'🏦', color:'#002C5F' },
      { label:'실차감', val:boFmt(org.budget.deducted)+'원', icon:'📉', color:'#1D4ED8' },
      { label:'가점유', val:boFmt(org.budget.holding)+'원', icon:'⏳', color:'#B45309' },
      { label:'가용 잔액', val:boFmt(avail)+'원', icon:'✅', color:'#059669' },
    ].map(k=>`
    <div class="bo-kpi-card">
  ...
  <div class="bo-card" style="overflow:hidden">
    ...
    <table class="bo-table">
      ...
      <tbody>
        ${org.teams.map(t=>{
          const avail = t.budget.allocated - t.budget.deducted - t.budget.holding;
          const pct = ((t.budget.deducted+t.budget.holding)/t.budget.allocated*100).toFixed(0);
          return `<tr>
...
<div class="bo-fade">
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:24px">
    ${[
      { label:'운영계정', allocated:team.budget.allocated*0.6|0, deducted:team.budget.deducted*0.6|0, holding:team.budget.holding*0.6|0, color:'#1D4ED8' },
      { label:'기타계정', allocated:team.budget.allocated*0.2|0, deducted:team.budget.deducted*0.2|0, holding:team.budget.holding*0.2|0, color:'#7C3AED' },
      { label:'참가계정', allocated:team.budget.allocated*0.2|0, deducted:team.budget.deducted*0.2|0, holding:0, color:'#059669' },
    ].map(b=>{
  ...
  <div class="bo-card" style="overflow:hidden">
    ...
    <table class="bo-table">
      ...
      <tbody>
        ${MOCK_BO_APPLICATIONS.slice(0,3).map(a=>`
        <tr>
