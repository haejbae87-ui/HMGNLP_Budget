...
function _foManSetTab(id) {
  ...
  if (!c) return;
  const map = {
    "fo-overview": _foManOverview,
    "fo-scenarios": _foManScenarios,
    "fo-menus": _foManMenus,
    "fo-calc": _foManCalc,
    "fo-ia": _foManIA,
    "fo-process": _foManProcess,
  };
...
function _foManOverview() {
  ...
  <div style="padding:16px 20px;background:#EFF6FF;border-radius:12px;border:1px solid #BFDBFE">
    ...
    <p style="font-size:13px;color:#374151;line-height:1.8;margin:0">
      백오피스에서 설정한 <strong>교육지원 운영 규칙(패턴A~E)</strong>에 따라 학습자 화면이 동적으로 구성됩니다.<br>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
    <div style="background:#fff;border:1.5px solid #E5E7EB;border-radius:12px;padding:16px">
      ...
      ${[
        {
          role: "일반교육예산",
          desc: "일반교육 VOrg에 속한 팀 — 참가·운영 계정 사용",
          color: "#1D4ED8",
        },
        {
          role: "R&D교육예산",
          desc: "R&D VOrg에 속한 팀 — 통합 계정, 계획 필수(패턴A)",
          color: "#7C3AED",
        },
        {
          role: "복합 VOrg",
          desc: "HMC 내구기술팀처럼 일반+R&D 양쪽 VOrg 소속 가능",
          color: "#002C5F",
        },
        {
          role: "KIA 학습자",
          desc: "기아 일반교육 VOrg 기반 참가 신청",
          color: "#059669",
        },
        {
          role: "HAE 학습자",
          desc: "현대오토에버 VOrg — 전사·팀프로젝트 계정",
          color: "#BE123C",
        },
      ]
        .map(
  ...
  <div style="background:#F0FDF4;border-radius:12px;padding:16px;border:1px solid #A7F3D0">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px">
      ${[
        {
          t: "📚 직접 학습",
          flow: "개인직무 사외학습 등 본인 참여형. VOrg에 따라 일반/R&D 예산카드 + 프로세스 안내 표시",
          color: "#1D4ED8",
          pat: "패턴A·B",
        },
        {
          t: "🎯 교육 운영",
          flow: "이러닝/집합·워크샵/세미나 등 운영계정 기반. Step 2에서 VOrg 레이블 확인",
          color: "#7C3AED",
          pat: "패턴A·B",
        },
        {
          t: "📝 결과만 등록",
          flow: "이미 수료한 교육 이력 등록. 예산 차감 없이 학습이력만 기록",
          color: "#6B7280",
          pat: "패턴C·D",
        },
      ]
      ...
      steps: [
        {
          n: "1",
          t: "📚 직접학습 카테고리 선택",
          d: '행위 기반 카테고리에서 "📚 직접 학습" → "개인 직무 사외학습" 선택',
        },
        {
          n: "2",
          t: "예산 선택 + VOrg 레이블·프로세스 안내 확인",
          d: "VOrg 레이블(일반교육예산 등) 부착된 예산카드에서 참가계정 선택. 하단에 프로세스 패턴 안내 표시.",
        },
        {
          n: "3",
          t: "교육유형 선택",
          d: "정책 기반 허용 교육유형(이러닝·집합·세미나 등)에서 직접 선택",
        },
        {
          n: "4",
          t: "세부정보 입력 및 상신",
          d: "교육명·일정·금액·세부산출근거 입력 후 제출. 교육지원 운영 규칙 기반 결재선 자동 생성.",
        },
        {
          n: "5",
          t: "결과보고 작성",
          d: "수강 완료 후 수료증 첨부 및 실지출 입력. 승인 시 실 차감 확정.",
        },
      ],
    },
    {
      ...
      steps: [
        ...
        },
        {
          n: "2",
          t: "예산 선택: R&D교육예산 계정",
          d: 'VOrg 레이블(R&D교육예산) 부착된 통합계정 또는 참가계정 선택. 프로세스 안내로 "계획→신청→결과" 흐름 표시.',
        },
        {
          n: "3",
          t: "교육계획 선택 (교육유형 자동 세팅)",
          d: "사전 승인된 R&D 교육계획 목록에서 선택. 교육유형 자동 세팅. Step 3 Skip.",
        },
        {
          n: "4",
          t: "세부정보 입력 및 상신",
          d: "계획 데이터 기반으로 정보가 일부 자동 채워짐. 추가 정보 입력 후 제출.",
        },
        {
          n: "5",
          t: "결과보고 작성",
          d: "수강 완료 후 결과 입력. 승인 시 R&D예산 실차감 확정.",
        },
      ],
    },
    {
      ...
      steps: [
        {
          n: "1",
          t: "개인직무 사외학습 목적 선택",
          d: '신청 목적 선택 화면에서 "개인직무 사외학습" 선택',
        },
        {
          n: "2",
          t: "예산 선택: 예산 미사용",
          d: '3가지 예산 카드 중 "📝 예산 미사용(이력만 등록)" 선택',
        },
        {
          n: "3",
          t: "교육유형 선택",
          d: "자체 세미나·무료 강의·동영상 학습 등 유형 선택",
        },
        {
          n: "4",
          t: "학습내용 입력",
          d: "금액 입력 없이 학습명·내용·증빙 위주로 입력. 예산 잔액에 영향 없음.",
        },
        {
          n: "✅",
          t: "승인 즉시 이력 적재",
          d: "별도 결과보고 없이 승인 완료 시 학습이력 DB에 즉시 저장.",
        },
      ],
    },
    {
      title: "시나리오 4: 팀담당자의 집합교육 계획 수립",
      persona: "조O성 책임 (HMC 팀담당자)",
      badge: "#002C5F",
      pattern: "패턴 A: 계획 → 신청 → 결과",
      steps: [
        {
          n: "1",
          t: "교육계획 수립 (4단계 위저드)",
          d: "Step1 목적선택 → Step2 예산선택 → Step3 교육유형 → Step4 세부정보 입력",
        },
        {
          n: "2",
          t: "계획 상신",
          d: '계획서 상신 시 결재라인 자동 구성. 승인 결과가 "증빙 근거"로 확정.',
        },
        {
          n: "3",
          t: "교육 신청",
          d: "승인된 계획을 선택하여 팀 가용예산 범위 내에서 신청서 제출",
        },
        {
          n: "4",
          t: "결과 정산",
          d: "교육 완료 후 실지출 입력·증빙 첨부. 신청액 vs 실지출 차액 자동 처리.",
        },
      ],
    },
  ];
  ...
  const menus = [
    ...
    },
    {
      ...
      desc: `학습자가 교육 과정에 대한 <strong>예산 집행을 신청</strong>하는 화면입니다.<br>
             ...
             제출 시 교육지원 운영 규칙 → 금액 구간 → 결재자 자동 라우팅.`,
  ...
  <div style="background:#fff;border:1.5px solid #E5E7EB;border-radius:12px;padding:18px">
    ...
    <div style="display:flex;flex-direction:column;gap:8px;font-size:12px">
      ${[
        {
          cond: "계획 수립 화면",
          filter: 'usageScope.includes("plan") = true인 항목만 표시',
        },
        {
          cond: "교육 신청 화면",
          filter: 'usageScope.includes("apply") = true인 항목만 표시',
        },
        {
          cond: "결과 정산 화면",
          filter: 'usageScope.includes("settle") = true인 항목만 표시',
        },
        {
          cond: "국내 교육",
          filter: 'visibleFor === "both" || visibleFor === "domestic"',
        },
        {
          cond: "해외 교육",
          filter: 'visibleFor === "both" || visibleFor === "overseas"',
        },
      ]
        .map(
          (r) => `
      ...
      ${[
        {
          ...
          color: "#059669",
          bg: "#F0FDF4",
          desc: "금액 제한 없이 자유롭게 입력 가능합니다.",
        },
        {
          type: "⚠ Soft Limit",
          color: "#D97706",
          bg: "#FFFBEB",
          desc: "softLimit 초과 시 경고 메시지 표시. 사유 입력 후 진행 가능. 관리자 설정에 따라 추가 승인 필요 가능.",
        },
        {
          type: "🚫 Hard Limit",
          color: "#DC2626",
          bg: "#FEF2F2",
          desc: "hardLimit 초과 금액 입력 시 저장·제출이 시스템적으로 차단됩니다. 금액을 한도 이하로 조정해야 합니다.",
        },
      ]
        .map(
          (l) => `
...
function _foManIA() {
  const screens = [
    ...
    },
    {
      area: "교육계획",
      menu: "교육계획 목록",
      screen: "내 계획·팀 계획 탭 목록",
      type: "화면",
      diff: "★★☆",
      note: "team_view_enabled 탭 분기",
    },
    {
      area: "교육계획",
      menu: "계획 수립 위저드",
      screen: "Step 1 행위기반 목적 선택",
      type: "화면",
      diff: "★★☆",
      note: "📚/🎯/📝 카테고리 그룹핑 + 정책필터",
    },
    {
      area: "교육계획",
      menu: "계획 수립 위저드",
      screen: "Step 2 예산 선택 + VOrg·프로세스",
      type: "화면",
      diff: "★★★",
      note: "VOrg 레이블 + 프로세스 패턴 안내",
    },
    {
      area: "교육계획",
      menu: "계획 수립 위저드",
      screen: "Step 3 교육유형 선택",
      type: "화면",
      diff: "★★☆",
      note: "DB edu_type_items 기반",
    },
    {
      area: "교육계획",
      menu: "계획 수립 위저드",
      screen: "Step 4 세부정보 입력",
      type: "화면",
      diff: "★★★",
      note: "양식 필드 + 세부산출근거",
    },
    ...
    },
    {
      area: "결재",
      menu: "리더용 결재함",
      screen: "팀/조직 결재 대기·처리 목록",
      type: "화면",
      diff: "★★★",
      note: "리더 역할(팀장~본부장)만 노출",
    },
    {
      area: "매뉴얼",
      menu: "서비스 매뉴얼",
  ...
  const tree = [
    ...
    {
      label: "Step 1 목적 → Step 2 예산 → Step 3 유형 → Step 4 세부정보",
      indent: 4,
      icon: "·",
      bold: false,
    },
    ...
    <div style="background:white;border:1.5px solid #E5E7EB;border-radius:12px;padding:16px 20px">
      ${tree
        .map(
          (
            n,
          ) => `<div style="display:flex;align-items:center;gap:6px;padding:5px 0;padding-left:${n.indent * 20}px">
...
function _foManProcess() {
  ...
  ${flowRow(
    ...
    arrow(),
    flowBox("⑥ 교육지원 운영 규칙", "#002C5F", "#EFF6FF", "100px"),
  )}
  ${flowRow(
