...
async function renderChannelMgmt() {
  ...
  try {
    const { data } = await sb
      ...
      .eq("tenant_id", _chTenant);
    (data || []).forEach((c) => {
      courseCounts[c.channel_id] = (courseCounts[c.channel_id] || 0) + 1;
    });
  } catch (e) {}
  ...
  if (roleCodes.length > 0) {
    try {
      ...
      (data || []).forEach((r) => {
        roleMgrCounts[r.role_code] = (roleMgrCounts[r.role_code] || 0) + 1;
      });
    } catch (e) {}
  }
  ...
  const tenantSel = isPlatform
    ? `<select onchange="_chTenant=this.value;renderChannelMgmt()" style="${selStyle}">
      ${tenants
        ...
        .map(
          (t) =>
            `<option value="${t.id}" ${t.id === _chTenant ? "selected" : ""}>${t.name}</option>`,
        )
        .join("")}
    </select>`
    : `<span style="font-size:12px;font-weight:800;color:#374151;padding:6px 10px;background:#F3F4F6;border-radius:8px">${tenants.find((t) => t.id === _chTenant)?.name || _chTenant}</span>`;
      ...
      <table class="bo-table" style="font-size:12px">
        ...
        ${_chList
          .map((ch, i) => {
            ...
            const mgrCount = ch.role_code
              ? roleMgrCounts[ch.role_code] || 0
              : (ch.managers || []).length;
  ...
  if (ch.role_code) {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        _chSelectedMgrs = (users || []).map((u) => ({
          user_id: u.id,
          name: u.name,
          dept: u.organizations?.name || "",
        }));
      }
    } catch (e) {}
  } else {
...
function _chRenderMgrChips() {
  ...
  if (_chSelectedMgrs.length === 0) {
    el.innerHTML =
      '<span style="font-size:11px;color:#9CA3AF">아래에서 사용자를 검색하여 담당자를 추가하세요</span>';
    return;
  }
  el.innerHTML = _chSelectedMgrs
    .map(
      (m, i) =>
...
async function _chLoadInitialUsers() {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[Channel] FK join failed, fallback:",
    ...
    if (!data || data.length === 0) {
      el.innerHTML =
        '<div style="padding:16px;text-align:center;font-size:12px;color:#9CA3AF">등록된 사용자가 없습니다</div>';
      return;
    }
    _chRenderUserList(
      data,
    ...
    console.error("[Channel] user load error:", e);
    el.innerHTML =
...
async function _chSearchUsers() {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      el.innerHTML =
        '<div style="padding:12px;text-align:center;font-size:11px;color:#9CA3AF">검색 결과 없음</div>';
      return;
    }
...
function _chRenderUserList(users, subtitle) {
  ...
  if (!el) return;
  let html = subtitle
    ? `<div style="padding:6px 12px;font-size:10px;color:#9CA3AF;font-weight:700;background:#F9FAFB;border-bottom:1px solid #F3F4F6">${subtitle}</div>`
    : "";
  html += users
    .map((u) => {
...
async function _chEnsureRole(sb, channelId, channelName) {
  ...
  const tenants = typeof TENANTS !== "undefined" ? TENANTS : [];
  const tenantShort =
    tenants.find((t) => t.id === _chTenant)?.short_name || _chTenant;
...
async function _chDelete(idx) {
  ...
  if (courses && courses.length > 0) {
    alert(
      `하위 교육과정 ${courses.length}개가 있습니다. 과정을 먼저 삭제해주세요.`,
    );
    return;
  }
  if (
    !confirm(
      `"${ch.name}" 채널을 삭제하시겠습니까?\n연결된 역할 및 담당자 매핑도 삭제됩니다.`,
    )
  )
    return;
