      ...
      <div>
        ...
        <div style="display:flex;flex-wrap:wrap;gap:5px;margin-bottom:4px">
          ${r.accountCodes.map(c => {
            ...
            const sysBadge = sys === 'integrated' ? '<span style="font-size:8px;padding:1px 5px;border-radius:3px;background:#FEF3C7;color:#92400E;font-weight:800;margin-left:3px">통합</span>'
              : sys === 'platform' ? '<span style="font-size:8px;padding:1px 5px;border-radius:3px;background:#F0FDF4;color:#059669;font-weight:800;margin-left:3px">자체</span>'
            ...
            return `<code style="background:#FEF3C7;color:#92400E;padding:2px 8px;border-radius:5px;font-size:10px;font-weight:700">${c}</code>${sysBadge}` +
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ...
      <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:${i % 2 === 0 ? '#FAFAFA' : '#FFF'};border-radius:10px;border:1px solid #F3F4F6">
        ...
        <div style="display:flex;align-items:center;gap:4px;flex:1;flex-wrap:wrap">
          ${nodes.filter(n => n.type !== 'draft').map((n, j, arr) => {
            const st = NODE_TYPE_STYLES[n.type] || NODE_TYPE_STYLES.approval;
            const isConditional = n.activation === 'conditional';
            const condLabel = isConditional ? ' <span style="font-size:8px;color:#DC2626">(조건)</span>' : '';
            return `
  ...
  <div>
    ...
    <div style="display:flex;flex-wrap:wrap;gap:6px">
      ${accounts.map(a => {
        ...
        const sysIcon = sys === 'integrated' ? '🔗' : sys === 'platform' ? '⚡' : '🏢';
        return `
  ...
  <div>
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
      ...
      <div style="display:flex;gap:5px">
        ${Object.entries(NODE_TYPE_STYLES).map(([k, v]) => `<span style="font-size:9px;padding:2px 6px;border-radius:4px;background:${v.bg};color:${v.color};font-weight:700">${v.icon} ${v.label}</span>`).join('')}
      </div>
    </div>
...
function arAddRange(routingId) {
  const r = APPROVAL_ROUTING.find(x => x.id === routingId);
  if (!r) return;
  r.ranges.push({ max: null, label: '새 구간', nodes: [
    { type: 'draft', label: '기안자', order: 1 },
    { type: 'approval', label: '팀장', role: '팀장', final: true, order: 2 },
  ]});
...
function arAddNode(routingId, rangeIdx, nodeType) {
  ...
  const label = prompt(`${NODE_TYPE_STYLES[nodeType]?.label || nodeType} 노드의 이름을 입력하세요:`, '');
  if (!label) return;
  ...
  if (nodeType === 'approval') {
    ...
    const existingRules = (typeof APPROVAL_COOP_RULES !== 'undefined' ? APPROVAL_COOP_RULES : []).filter(rule => rule.tenantId === tenantId);
    if (existingRules.length > 0) {
      const ruleOptions = existingRules.map((rl, i) => `${i+1}. ${rl.name}`).join('\n');
      const choice = prompt(`조건 룰을 선택하세요:\n${ruleOptions}\n\n0. 새 룰 생성`, '1');
      if (choice === '0' || !choice) {
        const newRuleId = 'RULE-' + tenantId + '-' + Date.now();
        APPROVAL_COOP_RULES.push({
          id: newRuleId, name: label + ' 활성화 조건', tenantId,
          operator: 'OR', conditions: [{ field: 'soft_limit_exceeded', op: 'eq', value: true }]
        });
        newNode.conditionRuleId = newRuleId;
      } else {
        const selIdx = parseInt(choice) - 1;
        if (existingRules[selIdx]) newNode.conditionRuleId = existingRules[selIdx].id;
      }
    } else {
      const newRuleId = 'RULE-' + tenantId + '-' + Date.now();
      APPROVAL_COOP_RULES.push({
...
function arRemoveNode(routingId, rangeIdx, nodeIdx) {
  ...
  if (!r || !r.ranges[rangeIdx]) return;
  const nodes = r.ranges[rangeIdx].nodes;
  if (nodes[nodeIdx]?.type === 'draft') { alert('기안 노드는 삭제할 수 없습니다.'); return; }
  if (nodes.filter(n => n.type === 'approval').length <= 1 && nodes[nodeIdx]?.type === 'approval') {
    alert('최소 1개 승인 노드는 필요합니다.'); return;
  }
...
// ─── 룰 빌더 모달 ────────────────────────────────────────────────────────────
const RULE_FIELDS = [
  { value: 'soft_limit_exceeded', label: '세부산출근거 soft 초과', type: 'boolean' },
...
function arOpenRuleEditor(ruleId) {
  const rule = (typeof APPROVAL_COOP_RULES !== 'undefined' ? APPROVAL_COOP_RULES : []).find(r => r.id === ruleId);
  if (!rule) { alert('룰을 찾을 수 없습니다: ' + ruleId); return; }
...
function arCloseRuleModal() {
  ...
  if (_arEditId) {
    ...
    if (r) document.getElementById('ar-modal-body').innerHTML = _renderArEditor(r);
  }
}
...
function arUpdateCondVal(ruleId, idx, val) {
  ...
  else if (!isNaN(Number(val)) && val.trim() !== '') rule.conditions[idx].value = Number(val);
  else if (val.includes(',')) rule.conditions[idx].value = val.split(',').map(s => s.trim()).filter(Boolean);
...
function arRemoveCond(ruleId, idx) {
  const rule = APPROVAL_COOP_RULES.find(r => r.id === ruleId);
  if (!rule || rule.conditions.length <= 1) { alert('최소 1개 조건은 필요합니다.'); return; }
