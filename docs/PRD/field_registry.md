...
| `edu_category` | `edu_category` | 필수구분 | 전체 | ✗ | FO 기본정보 노출 | |
| `manager_info` | `manager_info` | 담당자 정보 | 전체 | ✗ | FO 기본정보 노출 | |
...
### 섹션 8: 시스템 산출/예산 필드 (System Computed)
> UI에 노출되지만, 사용자가 직접 입력하지 않고 시스템/공식에 의해 산출되는 금액 추적용 필드

| 표준 명칭 | DB/로직 키 | 설명 (공식) | 비고 |
|---|---|---|---|
| **최초배정액** | `allocated_amount` | 관리자가 1차로 지급한 실제 돈 | 기존 '배정액' |
| **약정금액** | `hold_amount` | 결재 진행중 임시 홀딩(점유) | 기존 '점유액' |
| **집행금액** | `actual_amount` | 결재 완료 후 사용 확정/차감 | 기존 '사용액' |
| **가용예산** | `available_amount` | `최초배정액 - (약정금액 + 집행금액)` | 기존 '잔액' |

---

### 섹션 9: 결과 단계 (Result — 교육 결과 입력)
