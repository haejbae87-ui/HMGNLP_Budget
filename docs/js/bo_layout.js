...
function boPlatformSelectTenant(tenantId) {
  ...
  groupSel.innerHTML =
    `<option value="">전체 VOrg (${groups.length}개)</option>` +
  ...
  if (role === "platform_admin") {
    ...
    const filteredGroups = (
      typeof VORG_TEMPLATES !== "undefined"
        ? VORG_TEMPLATES
        : EDU_SUPPORT_DOMAINS || []
    ).filter((g) => g.tenantId === selTenantId);
...
<div style="display:flex;align-items:center;gap:10px;padding:12px 18px;background:#FFFBEB;
  ...
  ${
    activeId
      ? `<span style="font-size:10px;background:#FEF3C7;color:#92400E;padding:2px 8px;border-radius:4px;font-weight:700">
    📊 현재: ${(typeof VORG_TEMPLATES !== "undefined" ? VORG_TEMPLATES : EDU_SUPPORT_DOMAINS || []).find((g) => g.id === activeId)?.name || activeId}
  </span>`
      : '<span style="font-size:10px;color:#9CA3AF">전체 VOrg 데이터 표시 중</span>'
  }
</div>`;
  }
...
function boIsolationGroupBanner() {
  return boRenderGroupContextBar();
}
function boIsolationGroupBanner_legacy() {
  return typeof boVorgBanner !== "undefined" ? boVorgBanner() : "";
}
...
function _getPendingCounts() {
  ...
  const myOps =
    typeof getPendingCountForPersona === "function"
      ? getPendingCountForPersona(p)
      : 3;
  return {
    // plan-mgmt 배지 제거: 수요예측기간 관리가 독립 메뉴로 분리됨 (2026-04-16)
    "my-operations": myOps,
  };
}
...
function _getMenus(persona) {
  ...
  if (persona.role === "budget_op_manager" || persona.role === "budget_hq")
    return BUDGET_OP_MENUS;
...
function _boGetCollapsedSections() {
  try {
    return JSON.parse(sessionStorage.getItem("boCollapsed") || "{}");
  } catch {
    return {};
  }
}
...
function boSwitchGnb(gnbCate) {
  ...
  const firstMenu = allMenus.find(
    (m) =>
      m.gnb === gnbCate &&
      (typeof checkMenuAccess === "function"
        ? checkMenuAccess(m.id, personaRoles, persona.accessMenus)
        : (persona.accessMenus || []).includes(m.id)),
  );
...
const _TENANT_GROUPS_DEF = [
  ...
  },
  {
    ...
    tenantId: "KIA",
    color: "#059669",
    keys: ["kia_tenant_admin", "kia_total_general", "kia_hq_general"],
  },
  {
    label: "HAE",
    tenantId: "HAE",
    color: "#7C3AED",
    keys: ["hae_tenant_admin", "hae_total", "hae_dept"],
  },
  {
    label: "로템",
    tenantId: "ROTEM",
    color: "#B45309",
    keys: ["rotem_tenant_admin", "rotem_total"],
  },
  {
    label: "엔지",
    tenantId: "HEC",
    color: "#0369A1",
    keys: ["hec_tenant_admin", "hec_total"],
  },
  {
    label: "제철",
    tenantId: "HSC",
  ...
  },
  {
    ...
    tenantId: "HTS",
    color: "#6D28D9",
    keys: ["hts_tenant_admin", "hts_total"],
  },
  {
    label: "글로비스",
    tenantId: "GLOVIS",
    color: "#0E7490",
    keys: ["glovis_tenant_admin", "glovis_total"],
  },
  {
    label: "차증권",
    tenantId: "HIS",
    color: "#9D174D",
    keys: ["his_tenant_admin", "his_total"],
  },
  {
    label: "케피코",
    tenantId: "KEFICO",
    color: "#1D4ED8",
    keys: ["kefico_tenant_admin", "kefico_total"],
  },
  {
    label: "ISC",
    tenantId: "HISC",
    color: "#374151",
    keys: ["hisc_tenant_admin", "hisc_total"],
  },
];
...
function renderBoHeader() {
  ...
  const currentGroup =
    _TENANT_GROUPS_DEF.find((g) => g.keys.includes(currentKey)) ||
    _TENANT_GROUPS_DEF[0];

  // --- 테넌트 셀렉트 ---
  const tenantOptions = _TENANT_GROUPS_DEF
    .map(
      (g) =>
...
function boNavigate(menuId) {
  ...
  const targetMenuDef = _getMenus(boCurrentPersona).find(
    (m) => m.id === menuId,
  );
  ...
  // isolation-groups 메뉴 제거됨 (2026-03-30: 교육지원 도메인 관리 → 제도그룹 종속으로 변경)
