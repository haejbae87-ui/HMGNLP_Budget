...
function _manSetTab(id) {
  ...
  if (!c) return;
  const map = {
    overview: _manOverview,
    personas: _manPersonas,
    menus: _manMenus,
    patterns: _manPatterns,
    "data-flow": _manDataFlow,
    tech: _manTech,
    devplan: _manDevPlan,
    ia: _manIA,
    db: _manDbSchema,
...
function _manOverview() {
  ...
  <div class="bo-card" style="padding:20px;background:linear-gradient(135deg,#EEF2FF,#F5F3FF)">
    ...
    <p style="font-size:13px;color:#374151;line-height:1.8;margin:0">
      ...
      예산계정 설정 → 교육지원 운영 규칙 설계 → 교육계획 수립 → 신청·결재 → 결과 정산의 전체 흐름을 지원합니다.<br>
      ...
      ${[
        {
          id: "HMC",
          name: "현대자동차",
          accs: "OPS·PART·ETC·RND (4계정, 통장22)",
          color: "#1D4ED8",
        },
        {
          id: "KIA",
          name: "기아",
          accs: "OPS·PART·ETC (3계정)",
          color: "#059669",
        },
        {
          id: "HAE",
          name: "현대오토에버",
          accs: "OPS·PART·CERT·EDU·TEAM (5계정)",
          color: "#7C3AED",
        },
        {
          id: "HSC",
    ...
    <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center">
      ${[
        ...
        },
        {
          n: "2",
          t: "교육지원 운영 규칙",
          s: "패턴A/B/C/D + 금액별 결재라인",
          c: "#7C3AED",
        },
        {
          n: "3",
          t: "통장 동기화",
          s: "조직별 통장 자동생성(isolated/shared)",
          c: "#0369A1",
        },
      ...
      ${i < a.length - 1 ? '<span style="color:#D1D5DB;font-size:18px">→</span>' : ""}`,
        )
        .join("")}
    </div>
  </div>
  ...
  <div class="bo-card" style="padding:18px;background:#F0FDF4;border:1.5px solid #A7F3D0">
    ...
    <div style="font-size:12px;color:#374151;line-height:1.8">
      <strong>1. 교육지원 운영 규칙 5단계 위저드:</strong> 7~8단계로 복잡했던 정책 생성 과정을 "정책 정의 → 정책 범위 → 패턴 → 양식 → 결재라인" 5단계로 대폭 간소화 설계 적용.<br>
...
function _manPersonas() {
  const ps = [
      ...
      steps: [
        {
          i: "🔐",
          t: "관리자 권한 매핑",
          d: "테넌트별 총괄 담당자에게 역할 부여. 역할 변경 시 메뉴 자동 연동.",
        },
        {
          i: "🖥️",
          t: "전사 모니터링",
    ...
    {
      name: "신O남 (HMC 일반예산 총괄)",
      role: "budget_global_admin",
      badge: "#1D4ED8",
      desc: "HMC 일반예산(HMC-OPS·PART·ETC) 소유. 마스터 설정·교육지원 운영 규칙·배정·계획 검토가 주요 업무.",
      steps: [
        {
          i: "💳",
          t: "예산 계정 설정 (6단계 위저드)",
        ...
        {
          i: "🔧",
          t: "교육지원 운영 규칙 설정 (5단계 위저드)",
          d: "정책정의→정책범위→패턴선택→양식연결→결재라인. 패턴A/B/C/D/E 선택.",
        },
        {
          i: "🏢",
          t: "교육조직 관리",
        ...
        },
        {
          i: "📐",
          t: "세부산출근거 관리",
          d: "운영(21종)/기타(7종) 항목별 기준단가·Soft/Hard Limit·사용단계 설정.",
        },
        {
          i: "📊",
          t: "결재라인 설정",
          d: "계정별 금액 구간→결재자 자동 라우팅. 예: 100만↓팀장전결 / 초과→본부장승인.",
        },
        {
          i: "📥",
          t: "나의 운영 업무",
          d: "[계획 승인 대기] / [신청 승인 대기] / [결과 정산 대기] 3탭 단계별 승인함.",
        },
      ],
      menus: [
        "대시보드",
      ...
      steps: [
        {
          i: "💳",
          t: "R&D 계정 설정",
          d: "HMC-RND 전용 6단계 설정. 패턴A(계획→신청→결과) 고통제 방식.",
        },
        {
          i: "🔧",
          t: "교육지원 운영 규칙 패턴A",
          d: "R&D교육예산 서비스: 계획 필수 → 계획 기반 신청 → 결과 정산 고정 흐름.",
        },
      ],
      menus: [
        "대시보드",
      ...
      steps: [
        {
          i: "🏗️",
          t: "격리그룹 관리",
          d: "테넌트 내 일반예산/R&D예산 격리그룹 생성 및 총괄담당자 지정.",
        },
      ],
      ...
      steps: [
        {
          i: "💳",
          t: "KIA 계정 설정",
          d: "운영(OPS)·참가(PART) 2개 계정. HMC 데이터와 isolationGroup으로 완전 격리.",
        },
        {
          i: "🔧",
          t: "교육지원 운영 규칙",
          d: "패턴B(신청→결과) 주요 사용. 참가비는 패턴C(후정산) 적용.",
        },
      ],
      menus: [
        "대시보드",
  ...
  const menus = [
    {
      ...
      name: "교육지원 운영 규칙 설정 ★",
      roles:
        "platform_admin(회사+그룹 필터) / tenant_global_admin(그룹 필터) / budget_op_manager(그룹 자동 고정)",
    ...
    },
    {
      ...
      desc: `계정 단위 금액 구간별 결재선을 설정합니다.<br>
            ...
            ※ 교육지원 운영 규칙의 금액별 결재라인과 함께 동작합니다.`,
...
function _manPatterns() {
  ...
  <div style="padding:14px 18px;background:#F5F3FF;border:1px solid #DDD6FE;border-radius:12px">
    <div style="font-size:12px;font-weight:900;color:#5B21B6;margin-bottom:6px">💡 교육지원 운영 규칙 패턴이란?</div>
    <div style="font-size:12px;color:#374151;line-height:1.7">
      백오피스 <strong>[교육지원 운영 규칙 설정]</strong>에서 A/B/C/D 패턴을 선택하면, 프론트오피스 학습자의 신청 흐름이 자동으로 구성됩니다.<br>
...
function _manDataFlow() {
  ...
  <div class="bo-card" style="padding:20px">
    ...
    <div style="padding:14px;background:#FFFBEB;border-radius:10px;border:1px solid #FDE68A;font-size:12px;color:#92400E;line-height:2">
      ...
      → 교육지원 운영 규칙 판별 (어떤 policyId인가?)<br>
      ...
      ${[
        {
          ...
          file: "service_policies (DB)",
          desc: "교육지원 운영 규칙: tenant_id, purpose, target_type, account_codes[], vorg_template_id, process_pattern, approval_config",
        },
        {
          name: "budget_accounts",
          file: "DB 테이블",
          desc: "예산 계정: code, name, tenant_id, uses_budget, virtual_org_template_id",
        },
        {
          name: "budget_account_org_policy",
          file: "DB 테이블",
          desc: "통장 정책: bankbook_mode(isolated/shared), bankbook_level, vorg_template_id",
        },
        {
          name: "org_budget_bankbooks",
          file: "DB 테이블",
          desc: "조직별 통장: org_id, account_id, vorg_group_id, template_id",
        },
        {
          name: "users + user_roles",
          file: "DB 테이블",
          desc: "FO 학습자: id, name, org_id, job_type + role_code(learner, team_leader 등)",
        },
        {
          name: "currentPersona",
          file: "fo_persona_loader.js",
          desc: "FO 페르소나 객체: id, name, tenantId, orgId, roles[], allowedAccounts[], budgets[]",
        },
        {
          name: "BO_PERSONAS",
          file: "bo_data.js + DB",
          desc: "BO 페르소나: role, tenantId, ownedAccounts, isolationGroup, accessMenus",
        },
        {
          name: "CALC_GROUNDS_MASTER",
          file: "bo_data.js",
          desc: "산출근거: usageScope[], visibleFor, softLimit, hardLimit, unitPrice",
        },
      ]
        .map(
          (d) => `
  ...
  <div class="bo-card" style="padding:20px">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${[
        ...
        {
          item: "결재 워크플로우",
          desc: "Camunda 또는 사내 전자결재 연동 (BPMN 프로세스 정의)",
        },
        {
          item: "SSO / IAM",
          desc: "현대차그룹 통합 계정 (OAuth2/SAML). 역할 자동 매핑",
        },
        {
          item: "알림 서비스",
        ...
        {
          item: "파일 스토리지",
          desc: "정산 증빙 업로드 (S3/Object Storage + CDN)",
        },
        {
          item: "Excel 추출",
          desc: "리포트 다운로드 (서버사이드 ExcelJS / Apache POI)",
        },
      ]
        .map(
          (d) => `
...
function _manDevPlan() {
  const phases = [
    {
      ...
      items: [
        {
          task: "SSO/IAM 연동 (OAuth2/SAML)",
          detail: "현대차그룹 통합계정 연동, 역할 자동 매핑",
          days: 5,
        },
        {
          task: "멀티테넌트 DB 스키마 설계",
          detail: "domainId 기반 Row-Level Security 설정",
          days: 5,
        },
        {
          task: "API Gateway 기본 구조",
          detail: "테넌트 컨텍스트 헤더 주입, 권한 미들웨어",
          days: 3,
        },
      ],
    },
    {
      phase: "Phase 2",
      title: "백오피스 마스터 데이터 관리",
      weeks: "3~5주",
      color: "#7C3AED",
      items: [
        {
          task: "예산 계정 관리 API",
          detail: "CRUD + 6단계 위저드 데이터 저장, isolationGroup 필터",
          days: 5,
        },
        {
          task: "교육조직 템플릿 API",
        ...
        {
          task: "세부산출근거 API",
          detail: "항목 CRUD, usageScope/visibleFor/softLimit/hardLimit",
          days: 3,
        },
        {
          task: "양식 빌더 API",
          detail: "Form Schema JSON 저장·버전관리, 공지사항/첨부파일 포함",
          days: 4,
        },
        {
          task: "교육지원 운영 규칙 API",
          detail: "5단계 위저드 저장, 패턴A~E, approvalConfig{}",
          days: 5,
        },
      ],
    },
    {
      ...
      items: [
        {
          task: "결재 워크플로우 엔진 연동",
          detail: "Camunda/사내 전자결재 BPMN 프로세스 정의",
          days: 7,
        },
        {
          task: "금액 구간별 결재자 자동 라우팅",
          detail: "approvalThresholds 평가 로직, 담당 결재자 배달",
          days: 4,
        },
        {
          task: "나의 운영 업무 API",
          detail: "계획/신청/결과 3탭 미결건 조회, 역할별 필터",
          days: 4,
        },
        {
          task: "알림 서비스",
    ...
    },
    {
      ...
      items: [
        {
          task: "교육신청 목록 화면 (리스트 우선)",
          detail: "상태 뱃지(결재진행중/승인/반려), 결과작성 버튼",
          days: 4,
        },
        {
          task: "교육신청 위저드 (목적→예산→유형→정보)",
          detail: "HAE/HSC/HMC/KIA 페르소나별 예산카드 분기",
          days: 6,
        },
        {
          task: "교육계획 위저드 (4단계)",
    ...
    },
    {
      ...
      items: [
        ...
        {
          task: "파일 스토리지 연동",
          detail: "S3/Object Storage 증빙 업로드, CDN 서명 URL",
          days: 3,
        },
        {
          task: "Excel 리포트 추출",
          detail: "ExcelJS/Apache POI 서버사이드 생성",
          days: 3,
        },
      ],
    },
    {
      ...
      items: [
        {
          task: "테넌트 격리 검증 (Pen Test)",
          detail: "Cross-tenant 데이터 접근 차단 검증",
          days: 3,
        },
        {
          task: "성능 테스트",
          detail: "동시 결재 처리, ERP 동기화 지연 케이스",
          days: 3,
        },
        {
          task: "UAT (사용자 수락 테스트)",
          detail: "페르소나별 시나리오 검증, 기획자 확인",
          days: 5,
        },
        {
          task: "운영 이관 & 문서화",
          detail: "배포 파이프라인 구성, 운영 매뉴얼 작성",
          days: 4,
        },
      ],
    },
  ...
  const totalDays = phases.reduce(
    (s, p) => s + p.items.reduce((ss, i) => ss + i.days, 0),
    0,
  );
  ...
  <div class="bo-card" style="padding:16px 20px;background:#F0FDF4;border:1.5px solid #A7F3D0">
    ...
    <div style="display:flex;flex-direction:column;gap:6px;font-size:12px;color:#374151">
      ${[
        {
          p: "P0 · 즉시",
          t: "메뉴 접근권한 + HAE 예산카드 + 교육지원 운영 규칙 필터",
          c: "#DC2626",
        },
        {
          p: "P1 · 2주내",
          t: "신청 리스트 화면 + 결과보고 버튼 + 양식 공지사항/첨부파일",
          c: "#D97706",
        },
        {
          p: "P2 · 1달내",
...
function _manIA() {
  const screens = [
    ...
    },
    {
      ...
      role: "플랫폼관리자",
      diff: "★★☆",
      note: "form_templates.fields 연동",
    },
    {
      area: "플랫폼총괄",
      menu: "교육유형 관리",
      screen: "교육 목적군·유형·세부유형 트리",
      type: "화면",
      role: "플랫폼관리자",
      diff: "★★★",
      note: "edu_type CRUD DB 연동",
    },
    // ── 테넌트 운영 ──
    {
      area: "테넌트운영",
    ...
    },
    {
      ...
      role: "테넌트",
      diff: "★★★",
      note: "tree jsonb + allowedJobTypes",
    },
    {
      area: "테넌트운영",
      menu: "산정기준 관리",
  ...
  const tree = [
    ...
    {
      label: "Step 0 정책 정의 ~ Step 4 결재라인",
      indent: 3,
      icon: "△",
      bold: false,
    },
    ...
    <div style="background:white;border:1.5px solid #E5E7EB;border-radius:12px;padding:16px 20px">
      ${tree
        .map(
          (
            n,
          ) => `<div style="display:flex;align-items:center;gap:6px;padding:4px 0;padding-left:${n.indent * 22}px">
...
function _manDbSchema() {
  const tables = [
    {
      ...
      color: "#1D4ED8",
      bg: "#DBEAFE",
      desc: "회사·법인 단위. 모든 데이터의 최상위 격리 기준",
      cols: ["id PK", "name", "short_name", "active", "created_at"],
    },
    {
      name: "isolation_groups",
      layer: "L2 격리그룹",
      color: "#16A34A",
      bg: "#D1FAE5",
      desc: "테넌트 내 예산계정 격리 그룹 (일반교육예산, R&D 등)",
      cols: ["id PK", "tenant_id FK", "name", "status", "created_at"],
    },
    {
      name: "organizations",
      layer: "L3 조직",
      color: "#B45309",
      bg: "#FEF3C7",
      desc: "HR 조직 트리 (본부>센터>실>팀). parent_id 자기참조",
      cols: [
        "id PK",
        "tenant_id FK",
        "parent_id FK(self)",
    ...
    },
    {
      ...
      color: "#7C3AED",
      bg: "#EDE9FE",
      desc: "시스템 사용자. 조직 소속 + 직군(job_type) 구분. FO 학습자의 원천",
      cols: [
        "id PK",
        "tenant_id FK",
        "emp_no",
    ...
    },
    {
      ...
      color: "#7C3AED",
      bg: "#EDE9FE",
      desc: "역할 정의 (platform_admin, learner, team_leader 등)",
      cols: ["code PK", "name", "descr", "level"],
    },
    {
      name: "user_roles",
      layer: "L4 역할",
      color: "#7C3AED",
      bg: "#EDE9FE",
      desc: "사용자-역할 N:M 매핑. scope_id로 적용 범위 제한. start_date/end_date로 권한 기간 설정.",
      cols: [
        "id PK",
        "user_id FK",
        "role_code FK",
    ...
    },
    {
      ...
      color: "#7C3AED",
      bg: "#EDE9FE",
      desc: "역할별 접근 가능 메뉴 ID 목록",
      cols: ["role_code FK", "menu_id"],
    },
    {
      name: "budget_accounts",
      layer: "L5 예산설정",
      color: "#0369A1",
      bg: "#E0F2FE",
      desc: "예산 계정. 테넌트별 OPS/PART/RND/CERT 등 계정 정의",
      cols: [
        "id PK",
        "code",
        "name",
    ...
    },
    {
      ...
      color: "#0369A1",
      bg: "#E0F2FE",
      desc: "통장 생성 정책. 계정별 bankbook_mode(isolated/shared) 설정",
      cols: [
        "id PK",
        "budget_account_id FK",
        "vorg_template_id FK",
        "bankbook_mode",
        "bankbook_level",
      ],
    },
    {
      ...
      color: "#0369A1",
      bg: "#E0F2FE",
      desc: "조직별 예산 통장. 각 팀/본부의 계정별 금고 역할",
      cols: [
        "id PK",
        "org_id",
        "org_name",
    ...
    },
    {
      ...
      color: "#9D174D",
      bg: "#FCE7F3",
      desc: "교육신청 양식. fields jsonb에 입력 필드 구성",
      cols: [
        "id PK",
        "tenant_id FK",
        "isolation_group_id FK",
    ...
    },
    {
      ...
      color: "#DC2626",
      bg: "#FEE2E2",
      desc: "교육계획. 정책 기반으로 생성. status: draft→submitted→approved",
      cols: [
        "id PK",
        "tenant_id FK",
        "account_code",
    ...
    },
    {
      ...
      color: "#DC2626",
      bg: "#FEE2E2",
      desc: "예산 원장. 모든 예산 변동을 tx_type으로 순차 기록",
      cols: [
        "id PK",
        "tenant_id FK",
        "account_code",
  ...
  const fkRows = [
    ...
    ["plans", "policy_id", "service_policies.id", "계획 → 교육지원 운영 규칙"],
  ...
  const jsonbRows = [
    [
      "virtual_org_templates",
      "tree",
      "본부/센터/실/팀 계층 + allowedJobTypes + budget",
    ],
    [
      "service_policies",
      "approval_config",
