...
async function _opLoadTenants(isSuperAdmin, myTenantId) {
  ...
  try {
    ...
    sel.innerHTML = `<option value="">전체 회사</option>` +
      (data || []).map(t => `<option value="${t.id}"${t.id === myTenantId ? ' selected' : ''}>${t.name}(${t.id})</option>`).join('');
    _opFilterTenantId = myTenantId;
    sel.value = myTenantId;
  } catch (e) { console.warn('[_opLoadTenants]', e.message); }
}
...
async function _opLoadVorgs(tenantId) {
  try {
    ...
    sel.innerHTML = `<option value="">전체 가상조직</option>` +
      _opVorgTemplates.map(v => `<option value="${v.id}">${v.name}</option>`).join('');
    _opFilterVorgId = '';
    const gsel = document.getElementById('op-filter-group');
    if (gsel) { gsel.innerHTML = `<option value="">전체 뱃지그룹</option>`; }
    _opFilterGroupId = '';
  } catch (e) { console.warn('[_opLoadVorgs]', e.message); }
}
...
async function _opLoadGroups(vorgId) {
  try {
    ...
    gsel.innerHTML = `<option value="">전체 뱃지그룹</option>` +
      _opBadgeGroups.map(g => `<option value="${g.id}">${g.name}</option>`).join('');
    _opFilterGroupId = '';
  } catch (e) { console.warn('[_opLoadGroups]', e.message); }
}
...
async function loadRequestsData() {
  ...
  try {
    ...
    requestsData = requestsData.map(r => ({ ...r, _badgeName: badgeNameMap[r.badge_id] || 'Unknown' }));

    renderReqs();
  } catch (e) {
    console.error('[loadRequestsData]', e);
    const tb = document.getElementById('req-body');
    if (tb) tb.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:20px;color:#ef4444">⚠️ 조회 실패: ${e.message}</td></tr>`;
  }
}
...
function renderProgs() {
  ...
  tbody.innerHTML = progressData.map(p => {
    let statStyle = 'color:#64748b', statText = p.status;
    ...
    if (p.status === 'IN_PROGRESS') { statStyle = 'color:#f59e0b;font-weight:700'; statText = '이수 중'; }
...
function switchOpTab(tabId) {
  ...
  const contents = { requests: 'op-content-requests', tracker: 'op-content-tracker' };
