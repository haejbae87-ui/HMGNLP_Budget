...
const PAGE_GUIDE_DATA = {
  ...
  'service-policy': {
    ...
    layers: [
      { icon: '🎯', label: '목적', color: '#7C3AED',
        content: '교육 서비스 흐름(계획→신청→결과), 예산 연동, 결재라인을 하나의 정책으로 통합 관리. 정책 1개 = 특정 그룹의 특정 예산으로 처리 가능한 교육의 전체 규칙.' },
      { icon: '📌', label: '주요 기능', color: '#1D4ED8',
        items: ['정책 위저드 8단계 (범위→정책명→목적→교육유형→패턴→대상조직→양식→결재라인)',
                '격리그룹·계정·목적·교육유형 조합별 독립 정책 구성',
                '프론트오피스 학습자 교육계획·신청에 적용될 정책 자동 매핑',
                '정책 상태 (운영 중 / 중지) 관리'] },
      { icon: '📏', label: '정책', color: '#059669',
        items: ['정책은 최소 1개의 예산 계정과 연결', '동일 계정에 여러 정책 가능 (교육유형으로 구분)',
  ...
  'isolation-groups': {
    ...
    layers: [
      { icon: '🎯', label: '목적', color: '#7C3AED',
        content: '테넌트(회사) 내부의 예산 격리 단위. 격리그룹마다 독립적인 예산 계정·총괄자·운영자·정책을 보유하여 조직 간 예산 비공개 원칙 적용.' },
      { icon: '📌', label: '주요 기능', color: '#1D4ED8',
        items: ['그룹 생성 / 수정 / 삭제', '예산총괄 담당자 및 예산운영 담당자 지정',
  ...
  'form-builder': {
    ...
    layers: [
      { icon: '🎯', label: '목적', color: '#7C3AED',
        content: '교육 계획·신청·결과 단계에 사용할 양식 템플릿을 생성. 목적·교육유형·단계를 태깅하여 서비스 정책 위저드에서 자동 필터링.' },
      { icon: '📌', label: '주요 기능', color: '#1D4ED8',
        items: ['양식 단계(type): plan / apply / result',
  ...
  'user-mgmt': {
    ...
    layers: [
      { icon: '🎯', label: '목적', color: '#7C3AED',
        content: '플랫폼 사용자의 등록·수정·역할 부여. 사용자는 tenants 단위로 관리되며 역할(role)에 따라 접근 가능한 메뉴가 다름.' },
      { icon: '📌', label: '주요 기능', color: '#1D4ED8',
        items: ['사번 자동채번 (현재 테넌트 최대사번+1)',
                '역할 부여: learner / budget_admin / budget_ops / tenant_admin / platform_admin',
                '조직 배정 (org_id)'] },
      { icon: '📏', label: '정책', color: '#059669',
        items: ['사용자는 자동으로 learner 역할 부여',
                '사번(emp_no) 중복 방지는 현재 테넌트 범위 내에서만 적용',
                'budget_admin 역할 사용자만 격리그룹 총괄 담당자 지정 가능'] },
      { icon: '🗄️', label: 'DB 힌트', color: '#0369A1',
        items: ['users: id, tenant_id, name, emp_no, email, org_id, job_type, status',
  ...
  'allocation': {
    ...
    layers: [
      { icon: '🎯', label: '목적', color: '#7C3AED',
        content: '격리그룹 내 예산 계정에 연도별 총 예산을 배정하고 부서별·팀별로 분배.' },
      { icon: '📌', label: '주요 기능', color: '#1D4ED8',
        items: ['연도·분기별 예산 배정', '격리그룹 → 계정 → 하위 조직 순 배정',
                '배정 vs 집행 현황 추적'] },
      { icon: '🗄️', label: 'DB 힌트', color: '#0369A1',
        items: ['budget_allocations: id, account_code, org_id, year, amount, allocated_at',
  ...
  'approval-routing': {
    ...
    layers: [
      { icon: '🎯', label: '목적', color: '#7C3AED',
        content: '정책에 등록된 결재라인 기준으로 신청 건이 자동으로 담당 결재자에게 배달됨.' },
      { icon: '📌', label: '주요 기능', color: '#1D4ED8',
        items: ['금액 구간별 결재자 지정', '계획·신청·결과 단계별 독립 결재라인',
                '최종 결재자 승인 시 상태 자동 변경'] },
      { icon: '🗄️', label: 'DB 힌트', color: '#0369A1',
        items: ['service_policies.approval_config: {plan/apply/result: {thresholds:[{maxAmt, approverKey}], finalApproverKey}}',
  ...
  'fo-plan': {
    ...
    layers: [
      { icon: '🎯', label: '목적', color: '#7C3AED',
        content: '학습자가 연간·분기 교육 계획을 수립하여 예산 총괄에게 사전 승인 요청. 계획이 승인된 항목만 신청 단계로 진행 가능 (패턴 A).' },
      { icon: '📌', label: '주요 기능', color: '#1D4ED8',
        items: ['교육 목적 선택 → 예산 계정 → 교육유형 (정책 기반 자동 필터)',
                '정책에 연결된 계획 양식(form_type=plan) 작성',
                '제출 시 결재라인 자동 라우팅'] },
      { icon: '📏', label: '정책', color: '#059669',
        items: ['SERVICE_POLICIES_FO 기반으로 목적·예산·교육유형 필터링',
                '계획 단계가 없는 패턴(B/C/D/E)은 계획 메뉴 비활성'] },
      { icon: '🗄️', label: 'DB 힌트', color: '#0369A1',
        items: ['plans: id, user_id, policy_id, form_id, status, estimated_cost, created_at',
  ...
  'fo-apply': {
    ...
    layers: [
      { icon: '🎯', label: '목적', color: '#7C3AED',
        content: '승인된 계획 또는 직접 신청(패턴 B/D/E)으로 실제 교육 참가를 신청하고 결재 처리.' },
      { icon: '📌', label: '주요 기능', color: '#1D4ED8',
        items: ['계획 연계 신청(패턴 A) 또는 직접 신청(패턴 B)',
                '비용 상세(훈련비·숙박·식비 등) 입력',
                '신청 양식(form_type=apply) 작성 후 제출'] },
      { icon: '📏', label: '정책', color: '#059669',
        items: ['패턴 A: plan.status=approved 인 계획만 신청 가능',
                '신청 금액 구간에 따라 결재자 자동 배정'] },
      { icon: '🗄️', label: 'DB 힌트', color: '#0369A1',
        items: ['applications: id, plan_id (nullable), user_id, policy_id, form_id, total_cost, status',
  ...
  'fo-result': {
    ...
    layers: [
      { icon: '🎯', label: '목적', color: '#7C3AED',
        content: '교육 이수 후 결과를 보고하여 예산 집행을 확정하고 학습 이력을 저장.' },
      { icon: '📌', label: '주요 기능', color: '#1D4ED8',
        items: ['수료 증빙 업로드', '실 비용 정산 입력', '결과 양식(form_type=result) 작성'] },
      { icon: '🗄️', label: 'DB 힌트', color: '#0369A1',
        items: ['results: id, application_id, actual_cost, completion_date, attachments[]',
                'ledger에 집행 반영'] }
    ]
  }
};
...
function _renderPageGuideDrawer() {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;z-index:1000;opacity:0;transition:opacity .25s';
  overlay.innerHTML = `
