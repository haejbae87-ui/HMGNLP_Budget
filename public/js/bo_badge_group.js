    ...
    <div style="background:#fff;border:1.5px solid #E5E7EB;border-radius:12px;padding:14px 20px;margin-bottom:20px;display:flex;gap:12px;align-items:center;flex-wrap:wrap">
      ...
      <select id="bg-filter-tenant" onchange="onBgTenantChange()"
        ...
        <option value="">전체 회사</option>
          ...
          : `<span style="font-size:13px;font-weight:700;color:#374151;padding:8px 12px;background:#F1F5F9;border-radius:8px">${myTenantId}</span>`
      }
...
async function _bgLoadTenants(isSuperAdmin, myTenantId) {
  ...
  try {
    ...
    console.warn("[_bgLoadTenants]", e.message);
  }
}
...
async function _bgLoadVorgs(tenantId) {
  try {
    ...
    console.warn("[_bgLoadVorgs]", e.message);
  }
}
...
async function loadBadgeGroupData() {
  ...
  if (c)
    c.innerHTML = `<div style="text-align:center;padding:40px;color:#9CA3AF;grid-column:1/-1">불러오는 중...</div>`;
  try {
    let q = _sb()
      .from("badge_groups")
      .select("*")
    ...
    const c = document.getElementById("badge-groups-body");
    if (c)
      c.innerHTML = `<div style="text-align:center;padding:40px;color:#EF4444;grid-column:1/-1">⚠️ ${e.message}</div>`;
  }
}
...
async function _bgOpenGroupDetail(groupId) {
  ...
  try {
    ...
    if (allZero && data?.length)
      data.forEach((b, i) => {
        b.sort_order = (i + 1) * 10;
      });
    _bgDetailBadges = data || [];
  } catch (e) {
    alert("뱃지 조회 실패: " + e.message);
    return;
  }
    ...
    _bgDetailBadges.length === 0
      ...
      : _bgDetailBadges
          .map((b, i) => {
            ...
            const isFirst = i === 0,
              isLast = i === _bgDetailBadges.length - 1;
            return `
...
async function _bgMoveBadge(badgeId, direction) {
  ...
  const a = _bgDetailBadges[idx],
    b = _bgDetailBadges[swapIdx];
  const aOrd = (idx + 1) * 10,
    bOrd = (swapIdx + 1) * 10;
  try {
    ...
    a.sort_order = bOrd;
    b.sort_order = aOrd;
    [_bgDetailBadges[idx], _bgDetailBadges[swapIdx]] = [
      _bgDetailBadges[swapIdx],
      _bgDetailBadges[idx],
    ];
    _bgRenderGroupDetail();
  } catch (e) {
...
async function _bgDeleteBadge(badgeId) {
  if (
    !confirm(
      "삭제하시겠습니까?\n학습자 취득 이력이 있으면 심각한 문제가 발생할 수 있습니다.",
    )
  )
    return;
...
function openBadgeGroupModal() {
  const n = document.getElementById("bg-name"),
    d = document.getElementById("bg-desc");
  if (n) n.value = "";
  if (d) d.value = "";
    ...
    if (sel)
      sel.innerHTML =
        ...
        vorgBadgeTemplates
          .map((v) => `<option value="${v.id}">${v.name}</option>`)
          .join("");
    if (warn) warn.textContent = "※ badge 용도 제도그룹만 노출됩니다.";
  }
