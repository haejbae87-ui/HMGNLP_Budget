...
async function _enSearchUsers() {
    ...
    if (!sb) return;
    try {
        const { data } = await sb.from('users').select('id,name,department').eq('tenant_id', _enTenant).ilike('name', `%${q}%`).limit(20);
        ...
        <input type="checkbox" value="${u.id}" data-name="${u.name}" data-dept="${u.department || ''}"
        ...
        <div style="flex:1">
          ...
          <span style="font-size:11px;color:#6B7280;margin-left:8px">${u.department || ''}</span>
        </div>
