...
> **최초 작성**: 2026-04-18
> **최종 갱신**: 2026-04-24 (v1.6)
> **상태**: 🟡 기획 확정 — 개발 대기

### v1.6 주요 확정 사항 (2026-04-24)

> [!IMPORTANT]
> **다중 차수 선택 및 산출근거 통합**: 사용자의 편의성을 위해, 한 과정(Line Item) 내에서 **LMS 차수를 여러 개(다중 선택) 묶어서** 한 번에 예산 산출근거를 작성할 수 있도록 변경되었습니다. 예산 추적 단위는 '과정' 레벨로 관리됩니다. (기존 1 Line Item = 1 Session 제약 해제)
> **기사용 차수 Disable**: 여전히 이미 예산 집행 상신이 올라간 LMS 차수는 선택이 불가합니다.
> **동일 계정 + 동일 교육유형만 묶기 허용** (Q3-a 확정). 결재라인이 교육유형별로 세팅되므로 유형 혼합 시 충돌 발생.
>
> **부분 승인 불가** (Q3-b 확정). 신청서 단위 승인/반려만 허용. 계획 중 하나를 반려하면 신청서 전체 반려.
>
> **계획 담당자 ≠ 신청 작성자 허용** — 팀 대표가 팀의 여러 교육계획을 묶어 신청하는 개념. 예산 반영은 `application_plan_items.plan_id` 기준으로 각 계획에 개별 반영.
>
> **이어쓰기(회수/반려 후)**: 계획 목록 추가/삭제 **모두 가능** (Q7 확정). 금액 재계산 자동.
...
| F-002 | **과정별 차수 선택** | 선택한 각 계획에 대해 사용 가능한 차수를 다중 선택 (기 신청 차수 제외). 차수별로 예산을 분할하지 않고 통합 관리 | 🔴 HIGH |
| F-003 | **과정별 인원 입력** | 선택한 차수들의 총 참가자 구분 인원(현대/기아/그룹사 등)을 과정 단위로 합산하여 입력 | 🔴 HIGH |
| F-004 | **산출근거 계획 복사** | 계획의 산출근거를 자동 복사하여 **과정 단위(Line Item)**로 편집 가능. 차수별로 쪼개지 않음 | 🔴 HIGH |
| F-006 | **결재 1회** | N개 과정/계획이 포함된 1건의 신청서에 대해 리더 1회 결재 | 🔴 HIGH |
...
| 번호 | 기능 | 설명 | 우선순위 |
|------|------|------|:---:|
| F-010 | **패턴 기반 양식 결정** | process_pattern이 A인 정책 → Header + Line Items 양식 적용. 개인직무/교육운영 구분 없이 동일 구조 | 🔴 HIGH |
| F-011 | **적응형 Line Item 카드** | purpose에 따라 카드 내 필드 적응: 개인직무→인원=본인(구분 UI 숨김), 교육운영 집합/이러닝→📺과정-차수 연결 피커 노출, 기타 교육운영→과정 연결 숨김. 무예산 계정(`uses_budget=false`)→예산구분/정산 필드 숨김 | 🟡 MED |
...
│ │ 🏫 교육기관: 한국IT교육원                               │     │
│ │ 📺 LMS 과정: [채널 선택 ▼] [과정 선택 ▼]                │     │
│ │ 🎯 LMS 차수: ☑ 1차 ☐ 2차(기사용/Disable) ☑ 3차       │     │
│ │ 💳 예산구분: [H-교육 ▼]                     ← Line Item│     │
│ │ ⚖️ 정산방식: [인원비율정산 ▼]                 ← Line Item│     │
│ │ 👥 예상 인원 (선택한 차수 총합):                         │     │
│ │    현대 10 | 기아 5 | 그룹사 3 | 기타 2 = 과정 총 합계 20명     │
│ │ 💰 산출근거 (과정 전체 통합 — 차수별 분리 안함):              │     │
│ │    강사료: 800,000 × 2 = 1,600,000                     │     │
│ │    교재비: 20,000 × 20 = 400,000                       │     │
│ │    소계: 2,000,000원                                   │     │
│ └────────────────────────────────────────────────────────┘     │
│ ┌─ 항목 ②: 외부 세미나 (워크숍/기타운영/개인직무) ────────────────┐     │
│ │ 📋 계획 연결: PLN-002 (데이터 분석 세미나)              │     │
│ │ 🏫 교육기관: 패스트캠퍼스                               │     │
│ │ 📅 교육 기간: 2027-06-10 ~ 2027-06-12                 │     │
│ │ ※ 차수 선택 없음 (LMS 과정 미개설 유형)                  │     │
│ │ 💳 예산구분: [H-정산 ▼]              ← 과정마다 다름!    │     │
│ │ ⚖️ 정산방식: [해당없음 ▼]            ← 정산 불필요!      │     │
│ │ 👥 예상 인원:                                          │     │
│ │    현대 8 | 기아 4 | 그룹사 0 | 기타 0 = 합계 12명      │     │
│ │ 💰 산출근거 (이 계획으로 이번에 쓸 예산):                  │     │
│ │    참가비: 150,000 × 12 = 1,800,000                    │     │
│ │    소계: 1,800,000원                                   │     │
│ └────────────────────────────────────────────────────────┘     │
│                                                             │
│ [+ 교육계획 추가하기]                                         │
...
CREATE TABLE application_plan_items (
  ...
  linked_sessions JSONB DEFAULT '[]',  -- 선택한 LMS 차수 목록 [{session_id, session_no, name}]
  -- ※ 개인직무/워크숍 등에서는 위 3개 컬럼 모두 NULL (연결 불필요)
...
-- 패턴 A인지는 서비스 정책(process_pattern)으로 판별
-- 예산 유무는 계정의 uses_budget 속성으로 판별
-- 별도 모드 컬럼이 필요 없음
```
...
3. [집합/이러닝 유형의 경우] LMS 과정 및 차수를 반드시 1개 이상 선택해야 하며, 이미 다른 신청에 사용된 LMS 차수는 비활성화됨.
```
...
> [!IMPORTANT]
> **패턴 A = Header + Line Items 양식. 개인직무/교육운영 구분 없음.**
...
function getApplySteps(processPattern) {
  if (processPattern === 'A') {
    // 사업계획 연결이 필요한 패턴 → Header + Line Items 양식
    return [
      { n: 1, label: "목적 선택" },
      { n: 2, label: "예산 선택" },
...
Step 3: 교육계획 구성 ← 핵심 Step (개인직무, 교육운영 모두 동일)
        ...
        │     ├── 차수 선택 (기 신청 차수 비활성)
        │     ├── 💳 예산구분 (uses_budget=true만 노출 / uses_budget=false 숨김)
        │     ├── ⚖️ 정산방식 (uses_budget=true만 노출 / uses_budget=false 숨김)
        │     ├── 📺 채널-과정-차수 연결 (v1.3):
...
| EC-11 | **결재 반려 후 재신청 — 편집 범위?** | 🟡 | ✅ **확정 (v1.5)**: 전면 수정 허용. 반려 시 **saved로 전환**되며, **특정 교육계획 추가/삭제, 특정 교육과정 추가/삭제, 특정 차수 추가/삭제** 모두 자유롭게 가능함. |
...
| ~~Q-MP3~~ | ~~결재 반려 시 Line Items 수정 범위?~~ | — | — | ✅ **해결 (v1.4)**: B안 — **과정 추가/삭제 모두 가능**. 반려 → saved 복귀 후 이어쓰기 시 계획 목록 자유 편집 |
...
| ~~Q-MP6~~ | ~~부분 취소~~ | — | — | ✅ **해결 (v1.4)**: A안 — **전체 취소만**. 신청서 단위 승인/반려/회수만 허용. 부분 수정 필요 시 전체 회수 → 수정 → 재상신 |
| ~~Q-MP7~~ | ~~과정별 결재 분기~~ | — | — | ✅ **해결 (v1.4)**: A안 — **전체 단위 결재만**. 리더가 특정 과정만 반려 불가. 신청서 전체를 반려하고 재수정 요청 |
...
| 2026-04-20 | **v1.4** — 사용자 Q3/Q7 확정 반영. **동일계정+동일교육유형 제약 확정**. **부분승인 불가 확정**(Q-MP6 A안, Q-MP7 A안). **이어쓰기 시 계획 추가/삭제 가능**(Q-MP3 B안). **팀 대표 작성 개념 도입**(계획 담당자 ≠ 신청 작성자 허용). EC-11 갱신 | AI (비판적 기획자) |
| 2026-04-24 | **v1.5** — 사용자 요구사항 반영하여 **'차수(Round)'의 개념을 전면 수정**. 차수는 교육계획의 속성이 아니라 LMS 교육과정의 속성(Session)임. 집합/이러닝 유형은 LMS 채널>과정>차수를 선택하고 기사용 차수는 Disable 됨. 워크숍/기타/개인직무 등은 차수 개념 없이 산출근거만 입력. | AI (비판적 기획자) |
| **2026-04-24** | **v1.6** — 사용자 피드백 반영. **차수별 산출근거 분리 → 과정 단위 산출근거 통합**. 한 Line Item 내에서 복수의 LMS 차수를 다중 선택하고, 해당 과정 전체에 대한 예산을 한 번에 입력하도록 수정 (`session_id` → `linked_sessions JSONB` 롤백). | AI (비판적 기획자) |
