  ...
  if (sb && !_dashDbLoaded) {
    ...
    try {
      const [appsRes, plansRes] = await Promise.all([
        sb
          .from("applications")
          .select("id,edu_name,edu_type,amount,status,created_at,detail")
          .eq("applicant_id", pid)
          .eq("tenant_id", tid)
        ...
        sb
          .from("plans")
          .select("id,edu_name,amount,status,account_code,created_at")
          .eq("applicant_id", pid)
          .eq("tenant_id", tid)
          .order("created_at", { ascending: false })
          .limit(50),
...
function _renderDashboardHtml() {
  ...
  const totalHours = completedApps.reduce(
    (s, a) => s + Number(a.detail?.hours || 0),
    0,
  );
  const totalDone = completedApps.length;
  const totalSpent = completedApps.reduce(
    (s, a) => s + Number(a.amount || 0),
    0,
  );

  // 예산 잔액: currentPersona.budgets (하드코딩 마스터 데이터)
  const totalBal = (currentPersona?.budgets || []).reduce(
    (s, b) => s + (b.balance - b.used),
    0,
  );
          ...
          budgets.length > 0
            ? budgets
              ...
              <div class="flex justify-between text-[10px] text-gray-400 mt-1">
                ...
                .join("")
            : '<div class="text-xs text-gray-400 text-center py-4">배정된 예산이 없습니다</div>'
        }
      </div>
    </div>
