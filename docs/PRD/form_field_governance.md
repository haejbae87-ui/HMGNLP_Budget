...
| F-007 | L2 필드 최대 10개 제한 | 테넌트당 L2 확장 필드 생성 수 10개로 제한 (`_FB_L2_MAX = 10`) | 🟡 MED | ✅ |
| F-008 | DB 필드 카탈로그 | `form_field_catalog`, `field_options`, `field_dependencies`, `form_field_bindings` 테이블 | 🔴 HIGH | ✅ |
| F-009 | 토스트 알림 | 선행 필드 자동 추가 시 화면 하단 토스트 메시지 표시 (3초 후 자동 소멸) | 🟡 MED | ✅ |
| F-010 | 프론트오피스 필수 검증 | 필수 필드 미입력 시 임시저장 ✅ / 최종 상신 ❌ | 🔴 HIGH | ✅ |
| F-011 | select 필드 팔레트 표시 | 팔레트에서 select 타입 필드에 ▼ 배지, L2 필드에 L2 배지 표시 | 🟢 LOW | ✅ |
| F-012 | canonical_key 매핑 | 모든 필드에 영문 snake_case `canonicalKey` 부여. 그룹사 통계 집계 기준 | 🔴 HIGH | ✅ |
