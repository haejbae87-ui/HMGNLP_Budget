...
| 원칙 | 설명 |
|------|------|
| **패턴이 양식을 결정** | 프로세스 패턴(A~C) 선택 → 해당 단계의 양식 편집기 자동 표시 |
...
| **자비학습 = 예산 사용** | 자비학습 환급은 예산 사용 계정(`uses_budget=true`)에 해당. 무예산 계정(`uses_budget=false`)에서는 실비 입력 차단 |
...
│   │   │   ├── 🔘 장소유형 · 예상차수 · 교육일수
│   │   │   └── 🔘 교육기간 · 교육기관
│   │   ├── 💰 비용항목 ← uses_budget=false 시 전체 disable
│   │   │   ├── 🔘 계획액 · 세부산출근거
│   │   │   └── 🔘 기대효과
...
│   └── 📄 결과 양식 편집기 (동일 구조)
├── [패턴B: 신청→결과] → 신청 + 결과만
└── [패턴C: 신청→결과 (사후 정산형)] → 신청 + 결과만

※ 예산 유무는 패턴이 아닌 계정의 uses_budget 속성이 결정
※ uses_budget=false 계정 선택 시 비용 관련 필드 전체 disable
```
...
| **2026-05-05** | **패턴 3종 표준화**: 프로세스 패턴 A~E(5가지) → A~C(3가지)로 단순화. 패턴 D(신청 단독)/E(신청→결과+비용 disable) 폐기. 예산 유무를 패턴이 아닌 계정의 `uses_budget` 속성으로 결정하도록 전환. §9(F 핵심 설계 원칙), §9(F 인라인 양식 편집기 UI 구조) 갱신 | AI |
