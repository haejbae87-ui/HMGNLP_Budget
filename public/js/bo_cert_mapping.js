...
// 제도그룹 목록 로드 (purpose = 'cert' 또는 '자격증')
async function _cmLoadTemplates() {
  ...
  try {
    ...
    console.error("자격증 제도그룹 로드 실패:", err);
  }
}
...
function _cmRenderFilterArea() {
  ...
  // 제도그룹 필터링
  let filteredTpls = _cmTemplates.filter(
    (t) => t.tenant_id === _cmSelectedTenant,
  );
  ...
  if (
    _cmSelectedTplId &&
    !filteredTpls.find((t) => t.id === _cmSelectedTplId)
  ) {
  ...
  document.getElementById("cm-filter-area").innerHTML =
  ...
  if (!tpl) {
    mainEl.innerHTML = `
      <div style="padding:40px;text-align:center;background:#F9FAFB;border-radius:14px;border:1px dashed #D1D5DB">
        ...
        <div style="font-size:13px;font-weight:700;color:#6B7280">선택할 수 있는 자격증 제도그룹이 없습니다.</div>
      </div>
    `;
    return;
  }
...
function _cmRenderGroupCard(tpl, g, gi) {
    ...
    certs
      .map(
        (c, ci) =>
          ...
          "📜 " +
          (c.name || c) +
          " <button onclick=\"_cmRemoveCert('" +
          tpl.id +
          "'," +
          gi +
          "," +
          ci +
          ')" style="border:none;background:none;color:#C2410C;cursor:pointer;font-size:10px;padding:0;margin-left:4px;opacity:0.6" onmouseover="this.style.opacity=\'1\'" onmouseout="this.style.opacity=\'0.6\'">✕</button>' +
          "</span>",
      )
      .join("") ||
    '<span style="font-size:11px;color:#9CA3AF;padding:4px 0">맵핑된 자격증이 없습니다. 위 버튼을 눌러 추가하세요.</span>';
  ...
  return (
    ...
    g.name +
    "</span>" +
    '<span style="font-size:10px;padding:2px 8px;border-radius:5px;background:#FFF7ED;color:#C2410C;font-weight:700">' +
    certs.length +
    "개 자격증</span>" +
    "</div>" +
    "<button onclick=\"_cmAddCert('" +
    tpl.id +
    "'," +
    gi +
    ')" class="bo-btn-secondary bo-btn-sm" style="color:#C2410C;border-color:#FED7AA;background:#FFF">+ 자격증 추가</button>' +
    "</div>" +
