...
사용자: 내구기술팀 연구원 (HMC-1987 계정)
    ...
    │  ★ uses_budget=false 계정은 예산 없이 교육이력만 등록
    │  ★ 예산 유무는 패턴이 아닌 계정의 uses_budget 속성이 결정
    │
    └─ process_pattern='B' 또는 'C': 신청 → 결과 (사업계획 불필요)
```
...
| `process_pattern` | TEXT | 프로세스 패턴 (A~C) |
...
> **2026-05-05 확정**: 프로세스 패턴을 기존 5가지(A~E)에서 **3가지(A, B, C)**로 단순화.
> 예산 유무는 패턴이 아닌 **계정의 `uses_budget` 속성**이 결정한다.

| 패턴 | 흐름 | 결재 시점 | 비고 |
|------|------|----------|------|
| **A** | 사업계획 → 운영계획 → 신청 → 결과 | 사업계획 제출 시 결재 | 전체 프로세스 (예산 확보→운영 설계→집행→정산) |
| **B** | 신청 → 결과 | 신청 제출 시 결재 | 사업계획/운영계획 불필요, 직접 신청 |
| **C** | 신청 → 결과 (사후 정산형) | 신청 제출 시 결재 | B와 동일 흐름이나 사후 정산(실비 정산) 목적 |

> 💡 **예산 유무 결정 원칙**: 모든 패턴(A, B, C)에서 무예산 계정(`uses_budget=false`)이 존재할 수 있음.
> 무예산 계정 선택 시 비용 관련 필드(계획액, 산출근거, 배정액 등) 전체 disable.
...
| **2026-05-05** | **프로세스 패턴 3종 표준화**: A~E(5가지) → A~C(3가지)로 단순화. 패턴 D(결과만)/E(계획→결과)를 폐기. 예산 유무는 패턴이 아닌 계정의 `uses_budget` 속성이 결정하도록 원칙 변경. §3(파이프라인), §11(DB), §14(결재흐름) 전면 갱신 | AI |
