...
| 문서 상태 (DB) | 상태 배지 라벨 | Stepper 상태 | 표시되는 액션 버튼 (화면) |
|---|---|---|---|
| `draft` | 작성중 | 작성중(Active) | **[작성 폼 화면]** ✏️ 임시저장, 💾 저장(필수값 검증), ❌ 취소 |
| `saved` | 작성완료 | 상신대기(Active) | **[조회/확인 화면]** 📤 상신하기, ✏️ 수정 (수정 클릭 시 다시 작성 폼으로 돌아가며 임시저장/저장 버튼 부활) |
| `pending`/`submitted` | 결재대기 | 결재대기(Active) | **[조회/확인 화면]** ↩️ 회수하기 (팀장 승인 전까지만 가능), 📱 복제 |
| `team_approved`/`in_review` | 결재진행중 | 결재대기(Active) | **[조회/확인 화면]** 📱 복제 (회수 불가) |
| `approved`| 승인완료 | 승인완료(Done) | **[조회/확인 화면]** ▶ 이 계획으로 교육신청, 📉 배정액 축소, 📱 복제 |
| `rejected`| 반려 | 반려(Failed) | **[조회/확인 화면]** ✏️ 수정, 📤 상신하기 (재상신 유도) |

> **수정/상신 버튼 노출 원칙**: 결재 대기 중이거나 진행 중인 문서(`pending`, `submitted`, `team_approved`, `in_review`)는 수정 및 상신이 원천 차단됩니다. 해당 버튼들이 다시 활성화되기 위해서는 팀장/운영자가 문서를 반려(`rejected`)하거나, 작성자가 직접 상신을 회수(`recalled` → `saved`)해야 합니다.

> **결재함 정렬 정책**: 팀원용 및 리더용 결재함의 목록은 문서의 상신 일시(`submitted_at`)를 기준으로 최신순(내림차순) 정렬되어 노출됩니다. 상신 일시가 없는 경우 생성 일시(`created_at`)를 기준으로 정렬됩니다.
