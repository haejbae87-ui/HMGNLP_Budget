...
function applyNext() {
  ...
  if (nextStep === 3) {
    ...
    (async () => {
      ...
      // 2순위: form_templates DB 폴백 (form_config 미설정 계정만) -> [수정] 폴백 완전히 제거, 빈 기본 양식 렌더링 강제 (SSOT)
      if (!tpl) {
        console.warn('[applyNext] form_config 매칭 실패. 레거시 폴백 차단 및 기본 표준 양식(빈 inlineFields) 렌더링 강제');
        tpl = { 
          name: '기본 표준 양식 (전체 오픈)', 
          isInline: true, 
          inlineFields: {}, 
