...
다건 상신 시 결재선 규칙 (v1.1 Q-SUB1 확정):
- ✅ 동일 계정의 건만 하나의 상신 문서에 포함 가능
- 금액 합산 → 합산 기준으로 결재 구간 결정
- 다른 계정 건은 별도 상신 문서로 자동 분리
```

> **Q-SUB1**: ✅ **확정** — 다건 상신 시 **동일 예산계정의 건만** 하나의 상신 문서에 포함 가능.
> 서로 다른 계정의 건 선택 시 UI에서 경고 후 계정별 분리 상신 유도.
...
│  교육계획 목록 (plans list)                     │
│                                              │
│  [상태 필터: 전체 | 입력중 | 저장완료 | 상신중]   │
│                                              │
│  ☑  AI Foundations 교육계획     [saved] 500만  │
│  ☑  데이터분석 심화 계획         [saved] 350만  │
│  ☐  리더십 워크숍               [draft] 200만  │
│  ☐  Q1 컨퍼런스 결과보고         [submitted]    │
│                                              │
│  ─────────────────────────────────────────── │
│  2건 선택 │ 합계: 8,500,000원                  │
│                                              │
...
│                                              │
│  ※ saved 상태인 건만 선택 가능                  │
│  ※ draft는 저장 먼저 필요, submitted는 이미 상신 │
└──────────────────────────────────────────────┘
```
...
| EC-S02 | **계획+신청+결과 혼합 상신** | 🟡 | 허용 — 상신 문서에 `item_type` 혼합 가능. 단, 동일 결재선 적용 가능한 건만 |
...
| # | 시나리오 | 위험도 | 처리 |
|---|---------|:---:|------|
| EC-S06 | **팀장이 이미 1단계 승인한 후 회수 시도** | 🔴 | 회수 불가 — `in_review` 상태에서는 회수 차단. 결재자에게 반려 요청 안내 |
| EC-S07 | **회수 후 건 내용 수정 → 재상신** | 🟡 | 회수 시 건 status → `saved`. 수정 후 새 상신 문서 생성(기존 상신 문서는 `recalled` 이력 보존) |
...
| EC-S20 | **예산 차감 시점** | 🔴 | 상신 시 예약(hold) → 최종 승인 시 확정 차감. 회수/반려 시 예약 해제 |
...
| **Q-SUB2** | 상신 문서에 계획+신청+결과 혼합 허용? | EC-S02 | 🟡 | ⏳ 미결정 |
| **Q-SUB3** | 회수 가능 시점 — `submitted` 상태에서만? `in_review` 1단계까지? | EC-S06 | 🔴 | ⏳ 미결정 |
| **Q-SUB4** | 반려 후 재상신 시 새 상신 문서 생성? 기존 문서 재활용? | EC-S07 | 🟡 | ⏳ 미결정 |
...
| **Q-SUB6** | 예산 예약(hold) 메커니즘 구현 여부 — 상신 시 잔액 임시 차감? | EC-S20 | 🟡 | ⏳ 미결정 |
