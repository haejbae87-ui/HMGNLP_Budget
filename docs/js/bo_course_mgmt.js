  ...
  const tenantSel = isPlatform
    ? `<label style="font-size:10px;font-weight:700;color:#6B7280">테넌트</label>
        ...
        ${tenants
          ...
          .map(
            (t) =>
              `<option value="${t.id}" ${t.id === _crTenant ? "selected" : ""}>${t.name}</option>`,
          )
          .join("")}
      </select>`
    : `<span style="font-size:12px;font-weight:800;color:#374151;padding:6px 10px;background:#F3F4F6;border-radius:8px">${tenants.find((t) => t.id === _crTenant)?.name || _crTenant}</span>`;
...
async function _crSave() {
  ...
  if (editIdx !== "" && editIdx !== undefined) {
    ...
    await sb
      .from("edu_courses")
      .update({ title, channel_id: channelId, edu_type, content, status })
      .eq("id", c.id);
  } else {
    ...
    await sb
      ...
      .insert({
        ...
        content,
        status,
        created_by: boCurrentPersona.name || "",
      });
  }
...
async function _crDelete(idx) {
  ...
  if (sessions && sessions.length > 0) {
    alert(
      `하위 차수 ${sessions.length}개가 있습니다. 차수를 먼저 삭제해주세요.`,
    );
    return;
  }
