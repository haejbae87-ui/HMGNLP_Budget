...
function getSB() {
  const tid =
    (typeof currentPersona !== "undefined" && currentPersona?.tenantId) ||
    "HMC";
...
async function sbLoadTenants() {
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/tenants?select=*&active=eq.true&order=id`,
      {
        headers: {
    ...
    return data.map((t) => ({
      ...t,
      ...(_TENANT_COLORS[t.id] || _TENANT_COLORS.SYSTEM),
      budgetMode: "full",
    }));
  } catch (e) {
...
async function sbLoadServicePolicies(filters = {}) {
  try {
    ...
    if (filters.accountCode)
      q = q.contains("account_codes", [filters.accountCode]);
...
async function sbLoadRoleMenuPerms() {
  try {
    ...
    console.log(
      `[Supabase] ✅ role_menu_permissions 로드: ${Object.keys(map).length}개 역할, ${data.length}건`,
    );
    return map;
  } catch (e) {
    console.warn(
      "[Supabase] role_menu_permissions 로드 실패 → accessMenus 폴백 사용:",
      e.message,
    );
...
function _resolveRole(role) {
  return _ROLE_ALIAS[role] || role;
}
...
function checkMenuAccess(menuId, roles, fallbackMenus) {
  ...
  if (window._roleMenuPerms && Object.keys(window._roleMenuPerms).length > 0) {
    return (roles || []).some((role) =>
      window._roleMenuPerms[_resolveRole(role)]?.has(menuId),
    );
  }
...
function getAllowedMenuSet(roles, fallbackMenus) {
  if (window._roleMenuPerms && Object.keys(window._roleMenuPerms).length > 0) {
    ...
    (roles || []).forEach((role) => {
      (window._roleMenuPerms[_resolveRole(role)] || new Set()).forEach((m) =>
        allowed.add(m),
      );
    });
    return allowed;
...
function _buildBoPersonas(users, allRoles, orgs, igs) {
  ...
  const ACCESS_BY_ROLE = {
    ...
    budget_global_admin: [
      ...
      "vorg-coop-mgmt",
      "budget-account",
      "form-builder",        // Phase C: DB 폴백에도 접근 허용
      "calc-grounds",
      "pricing-dimensions",
      "forecast-period",
      "service-policy",
  ...
  (users || []).forEach((u) => {
    ...
    const sorted = [...nonLearnerRoles].sort(
      (a, b) =>
        (_ROLE_LEVEL[a.role_code] || 99) - (_ROLE_LEVEL[b.role_code] || 99),
    );
    ...
    const allowedAccounts = ownedAccounts.length
      ? ownedAccounts
  ...
  console.log(
    `[Supabase] ✅ BO_PERSONAS 로드: ${Object.keys(personas).length}명`,
  );
  return personas;
}
...
async function sbLoadEduTypes() {
  try {
    ...
    const itemsFlat = {};
    items.forEach((i) => {
      itemsFlat[i.id] = i.label;
    });

    console.log(
      `[Supabase] ✅ 교육유형 로드: 목적군 ${purposes.length}개, 세부유형 ${items.length}개`,
    );
    return { purposeMap, itemsFlat, purposes, groups, items };
  } catch (e) {
    console.warn(
      "[Supabase] 교육유형 로드 실패 → data.js fallback:",
      e.message,
    );
    return null;
  }
...
async function initSupabaseData() {
  ...
  try {
    ...
    if (vorgTemplates && vorgTemplates.length)
      window.VIRTUAL_EDU_ORGS = vorgTemplates;
    ...
    if (eduTypes && eduTypes.purposes && eduTypes.purposes.length) {
      ...
      window.PURPOSES = eduTypes.purposes
        .map((p) => eduTypes.purposeMap[p.id])
        .filter(Boolean);
    ...
    if (personas && Object.keys(personas).length > 0) {
      const pAdmin =
        typeof BO_PERSONAS !== "undefined"
          ? BO_PERSONAS["platform_admin"]
          : null;
    ...
    console.log(
      `[Supabase] ✅ DB 로딩 완료 - 테넌트:${tenants?.length}, 계정:${accounts?.length}, 격리그룹:${groups?.length}, 사용자:${personas ? Object.keys(personas).length : "mock"}`,
    );
    ...
    console.error(
      "[Supabase] ❌ 초기 로딩 실패. JS mock 데이터 사용:",
      e.message,
    );
    return false;
  }
...
async function sbLoadFormTemplates() {
  try {
    ...
    console.warn(
      "[Supabase] form_templates 로드 실패 → mock 데이터 사용:",
      e.message,
    );
    return [];
  }
...
async function sbLoadFieldDefinitions() {
  try {
    ...
    console.warn(
      "[Supabase] field_definitions 로드 실패 (JS 상수 사용):",
      e.message,
    );
    return null;
  }
...
async function sbGetBudgetBalance(accountCode, fiscalYear) {
  ...
  try {
    ...
    if (error) throw error;
    const available =
      (data.total_budget || 0) - (data.deducted || 0) - (data.holding || 0);
    return {
      accountCode,
      fiscalYear: year,
...
function _getBudgetBalanceMock(accountCode, year) {
  if (typeof ACCOUNT_BUDGETS !== "undefined") {
    const rec = ACCOUNT_BUDGETS.find(
      (b) => b.accountCode === accountCode && b.fiscalYear === year,
    );
    if (rec) {
      const available =
        (rec.totalBudget || 0) - (rec.deducted || 0) - (rec.holding || 0);
...
async function sbGetGroupBudgetSummary(ownedAccountCodes, fiscalYear) {
  ...
  return {
    ...
    deducted,
    holding,
    available: total - deducted - holding,
    accounts: results,
  };
}
