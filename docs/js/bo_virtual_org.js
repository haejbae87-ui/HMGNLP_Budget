...
let _voServiceType = "edu_support"; // 기본 제도유형
...
async function _voLoadRolesCheckboxes(selectedPurposes) {
  ...
  if (!selectedPurposes || selectedPurposes.length === 0) {
    box.innerHTML =
      '<span style="color:#9CA3AF;font-size:12px">제도유형을 먼저 선택하세요</span>';
    return;
  }
  box.innerHTML =
    '<span style="color:#9CA3AF;font-size:12px">로딩 중...</span>';
  try {
    ...
    if (!filtered.length) {
      box.innerHTML =
        '<span style="color:#9CA3AF;font-size:12px">해당 제도유형의 역할이 없습니다</span>';
      return;
    }
    ...
    box.innerHTML =
      '<span style="color:#EF4444;font-size:12px">로드 실패</span>';
  }
}
...
window._voEditTogglePurpose = async function (purpose) {
  ...
  await _voLoadEditRolesCheckboxes(
    [..._voEditSelectedPurposes],
    window._voEditCurrentRoleIds || [],
  );
};
...
async function voOpenEditTemplate(tplId) {
  ...
  // 기존 제도유형 복원
  const existTypes =
    Array.isArray(tpl.serviceTypes) && tpl.serviceTypes.length
      ? tpl.serviceTypes
      : [tpl.serviceType || tpl.purpose || "edu_support"];
  _voEditSelectedPurposes = new Set(existTypes);
  // 기존 역할 복원
  const existRoles =
    Array.isArray(tpl.ownerRoleIds) && tpl.ownerRoleIds.length
      ? tpl.ownerRoleIds
      : tpl.ownerRoleId
        ? [tpl.ownerRoleId]
        : [];
...
async function voConfirmEditTemplate() {
  ...
  if (!selectedTypes.length) {
    alert("제도유형을 하나 이상 선택해주세요.");
    return;
  }
  const checkedRoles = [
    ...document.querySelectorAll(".vo-edit-role-chk:checked"),
  ].map((cb) => cb.value);
  const tpl = _voMyTemplates.find((t) => t.id === tplId);
  if (!tpl) return;
  tpl.name = newName;
...
async function _voSaveToDB(tpl) {
  ...
  try {
    const sb = typeof _sb === "function" ? _sb() : null;
    if (!sb) {
      console.warn("[VOrg] Supabase client not ready");
      return false;
    }
    const payload = {
      id: tpl.id,
      ...
      service_type: Array.isArray(tpl.serviceTypes)
        ? tpl.serviceTypes.join(",")
        : tpl.serviceType || "edu_support",
      owner_role_id:
        Array.isArray(tpl.ownerRoleIds) && tpl.ownerRoleIds.length > 0
          ? tpl.ownerRoleIds[0]
          : tpl.ownerRoleId || null,
      owner_role_ids: Array.isArray(tpl.ownerRoleIds)
        ? tpl.ownerRoleIds
        : tpl.ownerRoleId
          ? [tpl.ownerRoleId]
          : [],
...
async function _voAutoSave(tpl) {
  ...
  if (ok) {
    ...
    toast.style.cssText =
      "position:fixed;bottom:24px;right:24px;background:#059669;color:#fff;padding:8px 18px;border-radius:10px;font-size:12px;font-weight:800;z-index:9999;box-shadow:0 4px 12px rgba(0,0,0,.2)";
  } else {
    toast.textContent = "⚠️ DB 저장 실패 (로컬 반영 유지)";
    toast.style.cssText =
      "position:fixed;bottom:24px;right:24px;background:#D97706;color:#fff;padding:8px 18px;border-radius:10px;font-size:12px;font-weight:800;z-index:9999;box-shadow:0 4px 12px rgba(0,0,0,.2)";
  }
...
async function renderVirtualOrg() {
  ...
  el.innerHTML =
    '<div class="bo-fade" style="padding:40px;text-align:center"><p style="color:#9CA3AF">로딩 중...</p></div>';
    ...
    if (sb) {
      ...
      if (!error && data) {
        _voMyTemplates = data
          ...
          .filter((t) => {
            if ((t.serviceType || "edu_support") !== _voServiceType)
              return false;
            // 조회 권한 필터
            ...
            const myCode =
              boCurrentPersona.roleCode ||
              `${_voTenantId}_${role.replace("_global_admin", "_admin")}`;
  ...
  if (
    !_voActiveTemplateId ||
    !_voMyTemplates.find((t) => t.id === _voActiveTemplateId)
  ) {
  ...
  const tenantName =
    tenants.find((t) => t.id === _voTenantId)?.name || _voTenantId;
  ...
  <div style="display:flex;align-items:center;gap:6px">
    <label style="font-size:11px;font-weight:700;color:#374151;white-space:nowrap">제도유형</label>
  ...
  const filterBar =
    isPlatform || isTenant || role === "budget_global_admin"
      ? `
  ...
  document.getElementById("bo-content").innerHTML =
    _renderVirtualOrgFull(filterBar);
}
...
function _renderVirtualOrgFull(filterBar) {
  ...
  const templateListHtml = _voMyTemplates
    .map((tpl) => {
  ...
  if (activeTpl) {
      ...
      <ul class="org-tree">
        ${groups
          .map((g, gIdx) => {
            ...
            const mgrKeys = g.managerPersonaKeys?.length
              ? g.managerPersonaKeys
              : g.managerPersonaKey
                ? [g.managerPersonaKey]
                : [];
            const mgrBadge =
              mgrKeys.length > 0
                ? mgrKeys
                    .map((k) => {
                      ...
                      return p
                        ? `<span style="font-size:10px;background:#EFF6FF;color:#1D4ED8;padding:2px 8px;border-radius:6px;font-weight:700">👤 ${p.name} (${p.dept})</span>`
                        : "";
                    })
                    .join(" ")
        ...
        <li style="margin-bottom:16px">
          ...
          <ul class="org-tree" style="padding-left:0">
            ${
              g.teams.length > 0
                ? `<li style="margin-bottom:8px"><div style="background:#F8FAFF;border:1.5px solid #E0E7FF;border-radius:10px;padding:10px 16px">
              <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
              ...
              ${g.teams
                .map((t, tIdx) => {
                  const jts =
                    t.allowedJobTypes && t.allowedJobTypes.length > 0
                      ? t.allowedJobTypes
                      : [];
                  const jtBadges = jts
                    .map(
                  ...
                  const allSelected =
                    allTypes.length > 0 &&
                    allTypes.every((jt) => jts.includes(jt));
                  ...
                  const jtPicker =
                    ...
                    allTypes
                      ...
                      .join(" ");
                  const subOrgBadge = t.includesSubOrgs
                    ? '<span style="font-size:9px;background:#DBEAFE;color:#1D4ED8;padding:1px 6px;border-radius:4px;font-weight:800">📂 하위 전체 포함</span>'
                    : "";
                  return `
    ...
    rightHtml = `
      <div style="height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:60px 0">
        ...
        <h3 style="font-weight:800;color:#374151;margin:0 0 8px">선택된 제도그룹이 없습니다</h3>
        <p style="font-size:13px;color:#6B7280;margin:0 0 24px">좌측 목록에서 제도그룹을 선택하거나 새로 생성해주세요.</p>
        <button class="bo-btn-primary" onclick="voOpenCreateTemplate()">+ 새 제도그룹 생성</button>
      </div>`;
  }
    ...
    <div style="width:280px;flex-shrink:0;display:flex;flex-direction:column">
      <div class="bo-card" style="flex:1;display:flex;flex-direction:column;overflow:hidden">
        <div style="padding:16px 20px;background:#F9FAFB;border-bottom:1px solid #E5E7EB;display:flex;justify-content:space-between;align-items:center">
          <span style="font-weight:800;font-size:14px;color:#111827">제도그룹 목록</span>
          <button class="bo-btn-primary bo-btn-sm" onclick="voOpenCreateTemplate()" style="padding:4px 10px;font-size:12px">+ 생성</button>
        </div>
        <div style="flex:1;overflow-y:auto">
          ${templateListHtml || '<div style="padding:32px 20px;text-align:center;color:#9CA3AF;font-size:12px">생성된 제도그룹이 없습니다</div>'}
        </div>
      </div>
    </div>
...
function voConfirmCreateTemplate() {
  ...
  // 체크된 역할들 수집
  const checkedRoles = [
    ...document.querySelectorAll(".vo-role-chk:checked"),
  ].map((cb) => cb.value);
...
async function voOpenEditTemplate(tplId) {
  ...
  if (ownerSel)
    ownerSel.innerHTML = await _voLoadRolesOptions(tpl.ownerRoleId || "");
  document.getElementById("vo-tpl-edit-modal").style.display = "flex";
}
...
async function voRemoveTemplate(id) {
  if (
    !confirm(
      "이 가상교육조직을 삭제하시겠습니까? (연결된 정책도 무효화될 수 있습니다.)",
    )
  )
    return;
  try {
    const sb = typeof _sb === "function" ? _sb() : null;
    if (sb) {
      await sb.from("virtual_org_templates").delete().eq("id", id);
    }
  } catch (e) {
    console.warn("[VOrg] 삭제 실패:", e.message);
  }
  _voMyTemplates = _voMyTemplates.filter((t) => t.id !== id);
  if (_voActiveTemplateId === id)
    _voActiveTemplateId = _voMyTemplates[0]?.id || null;
  document.getElementById("bo-content").innerHTML = _renderVirtualOrgFull();
}
...
function voOpenEditGroup(groupIdx) {
  ...
  const existing = g.managerPersonaKeys?.length
    ? g.managerPersonaKeys
    : g.managerPersonaKey
      ? [g.managerPersonaKey]
      : [];
...
function _voRenderMgrPicker(selectedKeys, filter = "") {
  ...
  if (chips) {
    if (selectedKeys.length === 0) {
      chips.innerHTML =
        '<span style="font-size:11px;color:#9CA3AF">선택된 담당자 없음</span>';
    } else {
      chips.innerHTML = selectedKeys
        .map((k) => {
          const p = BO_PERSONAS[k];
          if (!p) return "";
  ...
  const candidates = candidatesKeys.filter((k) => {
    ...
    return (
      ...
      (p.dept || "").toLowerCase().includes(lf)
    );
  });
...
function _voToggleMgr(cb) {
  if (cb.checked) {
    if (!_voSelectedMgrKeys.includes(cb.value))
      _voSelectedMgrKeys.push(cb.value);
  } else {
    _voSelectedMgrKeys = _voSelectedMgrKeys.filter((k) => k !== cb.value);
  }
  _voRenderMgrPicker(
    _voSelectedMgrKeys,
    document.getElementById("vo-mgr-search")?.value || "",
  );
}
...
function voConfirmCreateGroup() {
  ...
  if (_voEditGroupIdx !== null && _voEditGroupIdx !== undefined) {
    ...
    list.push({
      ...
      cooperators: [],
      budget: { total: 0, deducted: 0, holding: 0 },
      teams: [],
    });
  }
...
function _voRenderCoopList(cooperators) {
  ...
  if (cooperators.length === 0) {
    el.innerHTML =
      '<div style="padding:16px;text-align:center;color:#9CA3AF;font-size:12px;background:#F9FAFB;border-radius:8px">설정된 협조처가 없습니다</div>';
    return;
  }
  el.innerHTML = cooperators
    .map((c, i) => {
      const typeColor =
        c.coopType === "재경협조처"
...
function _voDeleteCoop(idx) {
  ...
  if (!activeTpl) return;
  const isRnd = activeTpl.tree.centers !== undefined;
  const g = (isRnd ? activeTpl.tree.centers : activeTpl.tree.hqs)[
    _voCoopGroupIdx
  ];
...
function voAddCoopTeam() {
  ...
  const role =
    document.getElementById("vo-coop-role-input")?.value.trim() || "협조";
  const coopType =
    document.querySelector('input[name="vo-coop-type"]:checked')?.value ||
    "교육협조처";
  const required =
    document.querySelector('input[name="vo-coop-required"]:checked')?.value ||
    "필수";
  if (!teamName) {
    alert("팀명을 입력하세요.");
    return;
...
function voSelectAllJobTypes(groupIdx, teamIdx) {
  ...
  const team = g.teams[teamIdx];
  const allTypes = getTenantJobTypes(boCurrentPersona.tenantId);
  const allSelected = allTypes.every((jt) =>
    (team.allowedJobTypes || []).includes(jt),
  );
...
async function voOpenAddTeam(groupIdx) {
  ...
  const grp = isRnd
    ? activeTpl.tree.centers[groupIdx]
    : activeTpl.tree.hqs[groupIdx];
  ...
  if (cont)
    cont.innerHTML =
      '<div style="text-align:center;padding:30px;color:#9CA3AF;font-size:12px">조직 로딩 중...</div>';
...
function _voIsAncestorSelected(nodeId, tree) {
  function findParentSelected(nodes, targetId, ancestors) {
    for (const n of nodes) {
      if (n.id === targetId)
        return ancestors.some((a) => _voSelectedTeams.has(a.id));
      const found = findParentSelected(n.children || [], targetId, [
        ...ancestors,
        n,
      ]);
...
function voRenderTree(existIds, filter = "", orgTree) {
  ...
  function renderNode(node, depth, ancestorSelected) {
    ...
    if (ex)
      badge =
        '<span style="font-size:10px;color:#6B7280;background:#E5E7EB;padding:2px 7px;border-radius:12px;font-weight:700">맵핑됨</span>';
    else if (parentCk)
      badge =
        '<span style="font-size:10px;color:#065F46;background:#D1FAE5;padding:2px 7px;border-radius:12px;font-weight:700">✓ 포함됨</span>';
    else if (ck && hasChildren)
      badge =
        '<span style="font-size:10px;color:#1D4ED8;background:#DBEAFE;padding:2px 7px;border-radius:12px;font-weight:700">📂 하위 전체 포함</span>';

    const rowBg = parentCk
      ? "#F0FDF4"
      : ck
        ? hasChildren
          ? "#EFF6FF"
    ...
    const childHtml = hasChildren
      ? (node.children || [])
          .map((c) => renderNode(c, depth + 1, ck || parentCk))
          .join("")
      : "";
  ...
  if (cont)
    cont.innerHTML =
      html ||
      '<div style="text-align:center;color:#9CA3AF;padding:40px;font-size:13px">조직 데이터가 없습니다</div>';
  _voUpdateSelCount();
}
...
function voToggleTeam(cb) {
  ...
  if (_voOrgTreeData && _voOrgTreeData.length > 0) {
    voRenderTree(
      existIds,
      document.getElementById("vo-team-search")?.value || "",
    );
  }
}
...
function voConfirmAddTeams() {
  if (!_voSelectedTeams.size) {
    alert("조직을 하나 이상 선택해주세요.");
    return;
  }
  ...
  document
    .querySelectorAll("#vo-tree-container input[type=checkbox]")
    .forEach((cb) => {
...
function voRenderTreeLegacy(budgetType, existIds, filter = "") {
  const orgGroups =
    budgetType === "rnd" ? REAL_ORG_TREE.rnd : REAL_ORG_TREE.general;
  const lf = filter.toLowerCase();
  const isRnd = budgetType === "rnd";

  const html = orgGroups
    .map((g) => {
      const vis = g.children.filter(
        (t) =>
          !lf ||
  ...
  if (cont)
    cont.innerHTML =
      html ||
      '<div style="text-align:center;color:#9CA3AF;padding:40px;font-size:13px">검색 결과가 없습니다</div>';
}
...
function voFilterTree() {
  ...
  _voSelectedTeams.forEach((id) => {
    const cb = document.querySelector(
      `#vo-tree-container input[value="${id}"]`,
    );
