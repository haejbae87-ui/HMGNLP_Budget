...
function renderBoAllocation() {
  ...
  const tenantName = TENANTS.find(t => t.id === persona.tenantId)?.name || '전체';
...
<div class="bo-fade">
  ...
  <div style="display:flex;gap:0;border-bottom:2px solid #E5E7EB;margin-bottom:24px" id="alloc-tabs">
    ${['📊 계정 예산 현황','➕ 기초·추가 배정','📋 팀 배분','↔ 이관','📜 변경 이력'].map((t,i)=>`
...
function showAllocTab(idx) {
  ...
  const fns = [renderAllocOverview, renderAllocEntry, renderTeamDist, renderAllocTransfer, renderAllocHistory];
  document.getElementById('alloc-content').innerHTML = fns[idx]();
}

// ─── 탭 1: 계정 예산 현황 (연도별 + 계정 선택 탭 + 가상조직 그룹핑) ──────────────
...
function renderAllocOverview(year) {
  ...
  const availableYears = [...new Set(allBudgets.map(ab => ab.fiscalYear || 2026))].sort((a,b)=>b-a);
  const myBudgets = allBudgets.filter(ab => (ab.fiscalYear || 2026) === _allocYear);

  // 선택 계정 초기값: 첫 번째 계정
  if (!_allocSelectedAbId || !myBudgets.find(ab => ab.id === _allocSelectedAbId)) {
...
<div style="margin-bottom:16px">
  ...
  <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px">
    ...
    ${availableYears.map(y => `<button onclick="switchAllocYear(${y})"
      ...
      font-weight:700;font-size:12px;cursor:pointer">${y}년</button>`).join('')}
  </div>
  <!-- 계정 선택 카드 패널 -->
  <div style="display:flex;gap:8px;flex-wrap:wrap">
    ${myBudgets.map(b => {
      ...
      const dist = TEAM_DIST.filter(t=>t.accountBudgetId===b.id).reduce((s,t)=>s+t.allocAmount,0);
  ...
  if (!ab) return topBarHtml + `<div style="padding:40px;text-align:center;color:#9CA3AF">계정을 선택하세요.</div>`;
...
function renderVorgManagerOverview() {
  const persona = boCurrentPersona;
  const myBudgets = getPersonaAccountBudgets(persona).filter(ab => (ab.fiscalYear||2026) === _allocYear);
  const vorg = getPersonaManagedVorg(persona);
  if (!vorg) return '<div style="padding:40px;text-align:center;color:#9CA3AF">관할 가상조직 정보가 없습니다.</div>';
  const isRnd = persona.budgetGroup === 'rnd';
  const orgIcon = isRnd ? '🔬' : '🏢';

  // 연도 선택기
  const availableYears = [...new Set(getPersonaAccountBudgets(persona).map(ab=>ab.fiscalYear||2026))].sort((a,b)=>b-a);
  const yearSel = availableYears.map(y=>'<button onclick="switchAllocYear('+y+')" style="padding:4px 14px;border-radius:8px;border:2px solid '+(y===_allocYear?'#059669':'#E5E7EB')+';background:'+(y===_allocYear?'#059669':'white')+';color:'+(y===_allocYear?'white':'#374151')+';font-weight:700;font-size:12px;cursor:pointer">'+y+'년</button>').join('');

  const acctCards = myBudgets.map(ab => {
    ...
    const td = TEAM_DIST.find(t => t.accountBudgetId===ab.id
      && (t.teamName===vorg.name || t.teamName.includes(vorg.name) || vorg.name.includes(t.teamName)));
    if (!td) {
      return '<div class="bo-card" style="overflow:hidden;margin-bottom:16px">'
        + '<div style="padding:12px 20px;background:#F9FAFB;border-bottom:1px solid #E5E7EB;display:flex;align-items:center;gap:10px">'
        + '<span style="font-size:14px">'+orgIcon+'</span>'
        + '<span style="font-weight:800;font-size:14px">'+vorg.name+'</span>'
        + '<code style="background:white;padding:1px 8px;border-radius:5px;font-size:10px;font-weight:900;border:1.5px solid '+(isSAP?'#93C5FD':'#FED7AA')+';color:'+(isSAP?'#1D4ED8':'#C2410C')+'">'+ab.accountCode+'</code>'
    ...
    const allocated = td.allocAmount, spent = td.spent, reserved = td.reserved;
    ...
    const teamRows = subTeams.map(rt => {
      ...
      const rBal=ra-rs-rr, rPct=ra>0?Math.min(((rs+rr)/ra)*100,100):0;
      const rC=rPct>=95?'#EF4444':rPct>=80?'#F59E0B':'#059669';
      const rBar = ra ? '<div style="height:5px;background:#E5E7EB;border-radius:99px;overflow:hidden;width:60px;margin:0 auto 2px"><div style="height:100%;background:'+rC+';width:'+rPct.toFixed(0)+'%"></div></div><span style="font-size:10px;color:'+rC+'">'+rPct.toFixed(0)+'%</span>' : '<span style="font-size:10px;color:#D1D5DB">—</span>';
      return '<tr style="background:white;border-top:1px solid #F1F5F9"><td style="padding:8px 16px 8px 40px"><div style="display:flex;align-items:center;gap:5px"><span style="color:#CBD5E1;font-size:11px">└─</span><span style="font-size:12px;font-weight:700;color:#374151">'+rt.name+'</span><span style="font-size:10px;color:#9CA3AF">실제팀</span></div></td>'
        ...
        + '<td style="text-align:center">'+rBar+'</td></tr>';
    }).join('');

    return '<div class="bo-card" style="overflow:hidden;margin-bottom:16px">'
      + '<div style="padding:14px 20px;background:'+(isRnd?'#F5F3FF':'#EFF6FF')+';border-bottom:1px solid #E5E7EB;display:flex;align-items:center;justify-content:space-between">'
      + '<div style="display:flex;align-items:center;gap:10px">'
      + '<span style="font-size:18px">'+orgIcon+'</span>'
      + '<div><div style="font-weight:900;font-size:15px">'+vorg.name+'</div><div style="font-size:10px;color:#6B7280">관할 가상'+(isRnd?'센터':'본부')+' · '+_allocYear+'년</div></div>'
      + '<code style="background:white;padding:2px 8px;border-radius:6px;font-size:11px;font-weight:900;border:1.5px solid '+(isSAP?'#93C5FD':'#FED7AA')+';color:'+(isSAP?'#1D4ED8':'#C2410C')+'">'+ab.accountCode+'</code>'
      + '</div><span style="font-size:11px;font-weight:900;color:'+pctColor+'">소진율 '+pct.toFixed(0)+'% '+(pct>=95?'🔴':pct>=80?'🟡':'🟢')+'</span>'
      + '</div>'
      ...
      + '<div style="text-align:center;background:#EFF6FF;padding:8px 16px;border-radius:10px"><div style="font-size:10px;color:#1D4ED8;font-weight:700">배분 받은 예산</div><div style="font-weight:900;font-size:18px;color:#1D4ED8">'+boFmt(allocated)+'</div></div>'
      + '<div style="color:#9CA3AF;font-size:14px">−</div>'
      + '<div style="text-align:center"><div style="font-size:10px;color:#EF4444;font-weight:700">집행</div><div style="font-weight:700;font-size:14px;color:#EF4444">'+boFmt(spent)+'</div></div>'
      + '<div style="color:#9CA3AF;font-size:14px">−</div>'
      + '<div style="text-align:center"><div style="font-size:10px;color:#B45309;font-weight:700">가점유</div><div style="font-weight:700;font-size:14px;color:#B45309">'+boFmt(reserved)+'</div></div>'
      + '<div style="color:#9CA3AF;font-size:14px">=</div>'
      + '<div style="text-align:center;background:'+(balance>0?'#F0FDF4':'#FEF2F2')+';padding:8px 16px;border-radius:10px;border:2px solid '+(balance>0?'#BBF7D0':'#FECACA')+'">'
      + '<div style="font-size:10px;color:'+(balance>0?'#059669':'#EF4444')+';font-weight:700">잔 액</div>'
      + '<div style="font-weight:900;font-size:18px;color:'+(balance>0?'#059669':'#EF4444')+'">'+boFmt(balance)+'</div></div>'
      + '</div>'
      + '<div style="margin-top:10px;height:8px;background:#E2E8F0;border-radius:99px;overflow:hidden"><div style="height:100%;background:'+pctColor+';width:'+pct.toFixed(0)+'%;border-radius:99px"></div></div>'
      + '</div>'
      // 하위 팀 테이블
      ...
      + '<tbody>'+(teamRows||'<tr><td colspan="6" style="text-align:center;padding:20px;color:#9CA3AF;font-size:12px">하위 팀 데이터가 없습니다</td></tr>')+'</tbody></table>'
      + '</div>';
  }).join('');

  return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:16px"><span style="font-size:11px;font-weight:700;color:#6B7280">📅 연도:</span>'+yearSel+'</div>'
    + '<div style="padding:10px 16px;background:#FFF7ED;border:1px solid #FED7AA;border-radius:10px;margin-bottom:16px;font-size:12px;color:#92400E;font-weight:600">👤 <b>'+persona.name+' '+persona.roleLabel+'</b> — 관할 조직(<b>'+vorg.name+'</b>)의 예산 현황만 표시합니다.</div>'
    + acctCards;
}
function renderAbDetail(ab) {
  ...
  const pct = totalBudget > 0 ? Math.min(((totalSpent+totalReserved)/totalBudget)*100, 100) : 0;
  ...
  const _isVorgMgr = typeof isVorgManager === 'function' && isVorgManager(ab._persona || boCurrentPersona);
  const _myVorgId = _isVorgMgr ? boCurrentPersona.managedVorgId : null;
  const _filteredGroups = _myVorgId ? vGroups.filter(vg => vg.id === _myVorgId) : vGroups;
  _filteredGroups.forEach(vg => {
    const matchedTds = flatTeams.filter(td => td.teamName.includes(vg.name) || vg.name.includes(td.teamName));
    ...
    <tr style="background:#F1F5F9;border-top:2px solid #CBD5E1">
      <td style="padding:10px 16px">
        <div style="display:flex;align-items:center;gap:6px">
          ...
          <div><div style="font-weight:900;font-size:13px">${vg.name}</div><div style="font-size:10px;color:#64748B">가상조직 소계 · ${vg.manager||'—'}</div></div>
        </div>
      </td>
    ...
    vg.teams.forEach(rt => {
      const rb = rt.budget||{};
      const ra=rb.allocated||0, rs=rb.deducted||0, rr=rb.holding||0;
      const rBal=ra-rs-rr, rPct=ra>0?Math.min(((rs+rr)/ra)*100,100):0;
      const rC=rPct>=95?'#EF4444':rPct>=80?'#F59E0B':'#059669';
      const rtPctBar = ra ? '<div style="height:5px;background:#E5E7EB;border-radius:99px;overflow:hidden;width:60px;margin:0 auto 2px"><div style="height:100%;background:'+rC+';width:'+rPct.toFixed(0)+'%"></div></div><span style="font-size:10px;color:'+rC+'">'+rPct.toFixed(0)+'%</span>' : '<span style="font-size:10px;color:#D1D5DB">—</span>';
      groupedRows.push(`
  ...
  flatTeams.filter(td=>!usedTdIds.has(td.id)).forEach(td=>{
    const b=td.allocAmount-td.spent-td.reserved, p=td.allocAmount>0?Math.min(((td.spent+td.reserved)/td.allocAmount)*100,100):0, c=p>=95?'#EF4444':p>=80?'#F59E0B':'#059669';
    groupedRows.push(`<tr><td style="font-weight:700">${td.teamName}</td><td style="text-align:right;font-weight:700">${boFmt(td.allocAmount)}</td><td style="text-align:right;color:#EF4444">${boFmt(td.spent)}</td><td style="text-align:right;color:#B45309">${boFmt(td.reserved)}</td><td style="text-align:right;font-weight:900;color:${c}">${boFmt(b)}</td><td style="text-align:center"><div style="height:6px;background:#E5E7EB;border-radius:99px;overflow:hidden;width:70px;margin:0 auto 2px"><div style="height:100%;background:${c};width:${p.toFixed(0)}%"></div></div><span style="font-size:10px;color:${c};font-weight:700">${p.toFixed(0)}%</span></td></tr>`);
  });

  const _distribRow = distributable > 0
    ? '<div style="margin-top:8px;display:flex;gap:8px"><button onclick="showAllocTab(2)" style="font-size:11px;padding:4px 12px;background:#059669;color:white;border:none;border-radius:8px;cursor:pointer;font-weight:700">📋 팀 배분하기 →</button><span style="font-size:11px;color:#059669;font-weight:600;align-self:center">미배분 '+boFmt(distributable)+'원 배분 가능</span></div>'
    : '';
  const _emptyRow = groupedRows.length === 0
    ? '<tr><td colspan="6" style="text-align:center;padding:24px;color:#9CA3AF;font-size:12px">아직 배분된 조직이 없습니다 — 팀 배분 탭에서 배분하세요</td></tr>'
    : groupedRows.join('');
  const _footHtml = flatTeams.length > 0
    ? ('<tfoot style="background:#F1F5F9;border-top:2px solid #CBD5E1"><tr>'
        + '<td style="padding:10px 16px;font-weight:900">전체 합계</td>'
        + '<td style="text-align:right;font-weight:900;padding:10px 16px;color:#1D4ED8">'+boFmt(distributed)+'</td>'
        + '<td style="text-align:right;color:#EF4444;padding:10px 16px">'+boFmt(totalSpent)+'</td>'
        + '<td style="text-align:right;color:#B45309;padding:10px 16px">'+boFmt(totalReserved)+'</td>'
        + '<td style="text-align:right;font-weight:900;padding:10px 16px;color:'+alertColor+'">'+boFmt(distributed-totalSpent-totalReserved)+'</td>'
        + '<td style="text-align:center;padding:10px 16px"><div style="height:7px;background:#E2E8F0;border-radius:99px;overflow:hidden;width:70px;margin:0 auto 2px"><div style="height:100%;background:'+alertColor+';border-radius:99px;width:'+pct.toFixed(0)+'%"></div></div><span style="font-size:10px;color:'+alertColor+';font-weight:700">'+pct.toFixed(0)+'%</span></td>'
        + '</tr></tfoot>')
    : '';
  return `
<div class="bo-card" style="overflow:hidden">
  ...
  <!-- 가상조직+팀 테이블 -->
  <div style="padding:10px 20px 4px"><div style="font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">조직별 배분 현황 (가상조직 → 실제팀)</div></div>
...
function renderAllocEntry() {
  ...
  if (!isOwner) return `<div style="padding:40px;text-align:center"><div style="font-size:40px">🔒</div><div style="font-weight:900;color:#374151">계정 오너만 사용 가능합니다</div></div>`;

  const myBudgets = getPersonaAccountBudgets(persona);
  const platformBudgets = myBudgets.filter(ab => ab.sourceType === 'platform');
    ...
    <div style="display:grid;gap:14px">
      <div>
        ...
        <select id="init-ab" style="width:100%;border:1.5px solid #FED7AA;border-radius:10px;padding:10px 14px;font-size:13px;font-weight:700">
          ...
          ${platformBudgets.filter(ab => ab.baseAmount === 0).map(ab =>
...
function previewAmt(inputId, previewId) {
  ...
  if (el) el.textContent = v > 0 ? `= ${boFmt(v)}원 (${(v/10000).toLocaleString('ko-KR')}만원)` : '';
}
...
function showAddSrcBadge() {
  ...
  if (badge && src) badge.innerHTML = `<div style="padding:8px 12px;border-radius:8px;font-size:11px;font-weight:700;background:${src==='sap_if'?'#EFF6FF':'#FFF7ED'};color:${src==='sap_if'?'#1D4ED8':'#C2410C'};border:1px solid ${src==='sap_if'?'#BFDBFE':'#FED7AA'}">
...
function submitInitBudget() {
  ...
  if (!abId || !amount) { alert('계정과 금액을 입력하세요.'); return; }
  const ab = ACCOUNT_BUDGETS.find(x => x.id === abId);
  if (!ab) return;
  if (ab.baseAmount > 0) { alert('이미 기초 예산이 등록된 계정입니다. 증액은 추가 배정을 이용하세요.'); return; }
  ...
  ACCOUNT_ADJUST_HISTORY.push({
    id:`AH${Date.now()}`, accountBudgetId:abId,
    date:new Date().toISOString().slice(0,10),
    type:'기초입력', amount, note, by:boCurrentPersona.name
  });
...
function submitAddBudget() {
  ...
  if (!abId || !amount || !reason) { alert('모든 항목을 입력하세요.'); return; }
  const ab = ACCOUNT_BUDGETS.find(x => x.id === abId);
  if (!ab) return;
  ...
  ACCOUNT_ADJUST_HISTORY.push({
    id:`AH${Date.now()}`, accountBudgetId:abId,
    date:new Date().toISOString().slice(0,10),
    type:'추가배정', amount, note:reason, by:boCurrentPersona.name
  });
  const acctName = ACCOUNT_MASTER.find(a=>a.code===ab.accountCode)?.name || ab.accountCode;
...
function renderTeamDist() {
  const persona = boCurrentPersona;
  const isOwner = (persona.ownedAccounts||[]).length > 0;
  const canDist = isOwner || (typeof isVorgManager==='function' && isVorgManager(persona));
  if (!canDist) return `<div style="padding:40px;text-align:center"><div style="font-size:40px">🔒</div><div style="font-weight:900;color:#374151">팀 배분은 계정 오너 또는 본부/센터 담당자만 가능합니다</div></div>`;

  // vorg manager 안내 배너
  const vmBanner = (!isOwner && isVorgManager(persona))
...
<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px">
  ...
  _filteredGroups.forEach(vg => {
    tableRows += `<tr style="background:#F1F5F9;border-top:2px solid #CBD5E1">
      <td style="padding:9px 14px" colspan="2">
        <div style="display:flex;align-items:center;gap:6px">
          ...
          <span style="font-size:10px;color:#64748B">가상조직</span>
        </div>
      </td>
      <td colspan="2"></td>
    </tr>`;
    vg.teams.forEach(rt => {
      ...
      allTeams.push({vgName:vg.name, teamName:rt.name, inputId, existing, currentAlloc:existing?.allocAmount||0});
      tableRows += `<tr>
...
function calcDistRemain() {
  ...
  let total = 0;
  teams.forEach(t => { total += Number(document.getElementById(t.inputId)?.value||0); });
  ...
  if (rv) { rv.textContent = boFmt(remain)+'원'; rv.style.color = remain<0?'#EF4444':'#059669'; }
  ...
  teams.forEach(t => {
    ...
    if (pv) pv.textContent = v>0 ? '→ '+(boFmt((t.currentAlloc||0)+v))+'원' : '';
  });
}

function submitBulkDist() {
  const teams = window._distAllTeams || [];
  ...
  teams.forEach(t => {
    ...
    else { TEAM_DIST.push({id:`TD${Date.now()}_${Math.random().toString(36).slice(2)}`,accountBudgetId:abId,teamName:t.teamName,allocAmount:v,spent:0,reserved:0}); }
...
function renderAllocTransfer() {
  const persona = boCurrentPersona;
  const isOwner = (persona.ownedAccounts||[]).length > 0;
  const canTransfer = isOwner || (typeof isVorgManager==='function' && isVorgManager(persona));
  if (!canTransfer) return `<div style="padding:40px;text-align:center"><div style="font-size:40px">🔒</div><div style="font-weight:900;color:#374151">이관은 계정 오너 또는 본부/센터 담당자만 가능합니다</div></div>`;
  ...
  const eligibleTeams = myBudgets.flatMap(ab =>
    TEAM_DIST.filter(t => t.accountBudgetId===ab.id
      && (!vmTeamNames || vmTeamNames.some(tn => t.teamName.includes(tn) || tn.includes(t.teamName))))
  );
...
function updateTrTeams() {
  ...
  if (vmTeams) {
    teams = teams.filter(t => vmTeams.some(tn => t.teamName.includes(tn) || tn.includes(t.teamName)));
  }
  const opts = `<option value="">— 팀 선택 —</option>`+teams.map(t=>`<option value="${t.id}">${t.teamName} (잔액: ${boFmt(t.allocAmount-t.spent-t.reserved)}원)</option>`).join('');
...
function submitTransfer() {
  ...
  if (!fromId || !toId || !amount || !reason || fromId===toId) { alert('모든 항목을 올바르게 입력하세요.'); return; }
  ...
  if (amount > bal) { alert(`잔액 부족: ${boFmt(bal)}원만 이관 가능합니다.`); return; }
  from.allocAmount -= amount;
  to.allocAmount += amount;
  const now = new Date().toISOString().slice(0,10);
  ACCOUNT_ADJUST_HISTORY.push({ id:`AH${Date.now()}a`, accountBudgetId:from.accountBudgetId, date:now, type:'이관출처', amount:-amount, note:`→ ${to.teamName}: ${reason}`, by:boCurrentPersona.name });
  ACCOUNT_ADJUST_HISTORY.push({ id:`AH${Date.now()}b`, accountBudgetId:to.accountBudgetId, date:now, type:'이관수신', amount:+amount, note:`← ${from.teamName}: ${reason}`, by:boCurrentPersona.name });
...
function renderAllocHistory() {
  ...
  const history = ACCOUNT_ADJUST_HISTORY
    .filter(h => myAbIds.includes(h.accountBudgetId))
    .sort((a,b) => b.date.localeCompare(a.date));

  const typeStyle = {
    'SAP_IF':   {bg:'#DBEAFE',c:'#1E40AF'},'기초입력':{bg:'#E0E7FF',c:'#4338CA'},
    '확정':     {bg:'#D1FAE5',c:'#065F46'},'추가배정':{bg:'#FEF3C7',c:'#92400E'},
    '이관출처': {bg:'#FEE2E2',c:'#991B1B'},'이관수신':{bg:'#F0FDF4',c:'#166534'},
  };
  ...
  <table class="bo-table">
    ...
    <tbody>
      ${history.map(h => {
        ...
        const ac = h.amount>0?'#059669':h.amount<0?'#EF4444':'#9CA3AF';
        return `<tr>
