...
function renderApprovalStepper(status, type) {
    ...
    const stepsHtml = steps.map((step, idx) => {
        ...
        if (isDone) {
            iconBg = '#059669'; iconBorder = '#059669';
            ...
            labelColor = '#059669'; lineColor = '#059669';
        } else if (isActive) {
            iconBg = '#EFF6FF'; iconBorder = '#2563EB';
            iconContent = `<div style="width:8px;height:8px;background:#2563EB;border-radius:50%"></div>`;
            labelColor = '#2563EB'; lineColor = '#E5E7EB';
        } else if (isFailed) {
            iconBg = '#FEE2E2'; iconBorder = '#DC2626';
            ...
            labelColor = '#DC2626'; lineColor = '#FECACA';
        } else {
            iconBg = '#F9FAFB'; iconBorder = '#D1D5DB';
            iconContent = `<div style="width:6px;height:6px;background:#D1D5DB;border-radius:50%"></div>`;
            labelColor = '#9CA3AF'; lineColor = '#E5E7EB';
        }
...
function _normalizeStepperStatus(st) {
    const map = {
        'draft': 'draft', '작성중': 'draft', '임시저장': 'draft',
        'pending': 'pending', '신청중': 'pending', '결재진행중': 'pending', '대기': 'pending', 'pending_approval': 'pending',
        'approved': 'approved', '승인완료': 'approved', '완료': 'approved', 'completed': 'approved',
        'rejected': 'rejected', '반려': 'rejected',
