  ...
  const rows = history
    ...
    <div style="display:flex;align-items:flex-start;gap:14px;padding:18px 20px;border-radius:16px;
      ...
      <div style="flex:1;min-width:0">
        ...
        ${h.applyStatus === '반려' ? `<div style="padding:8px 12px;border-radius:8px;background:#FEE2E2;border:1px solid #FECACA;font-size:11px;color:#DC2626;font-weight:700;margin-bottom:6px">?? 반려 사유: ${h.rejectReason || '예산 잔액 부족으로 반려되었습니다. 예산 계획 수립 후 재신청 바랍니다.'}</div>` : ''}
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">
          ${(() => {
            ...
            let a = "";
            if (isDraft) {
              a += `<button onclick="resumeApplyDraft('` + _aid + `')" style="padding:6px 14px;border-radius:8px;background:#1D4ED8;color:white;font-size:11px;font-weight:800;border:none;cursor:pointer">?? 이어쓰기</button>`;
