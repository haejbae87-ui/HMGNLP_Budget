...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
      ...
      if (!empNo || !password) { alert('아이디와 비밀번호를 입력해주세요.'); return; }
...
// 역할이 BO 접근 대상인지 확인
const _BO_ROLES = new Set(['platform_admin','tenant_admin','budget_admin','budget_ops']);
...
async function doLogin(empNo, password) {
  ...
  try {
    ...
    const _LEVEL = { platform_admin:1, tenant_admin:2, budget_admin:3, budget_ops:4, learner:99 };
    const sorted  = [...userRoles].sort((a, b) => (_LEVEL[a]||50) - (_LEVEL[b]||50));
