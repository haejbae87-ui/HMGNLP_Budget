// ─── bo_budget_policy_mgmt.js — 정책/조직맵핑 관리 (REFACTOR-1: bo_budget_master.js 분리) ───
...
function bmTenantSegment(activeTid, onchangeFn) {
  return `<div class="bo-segment">
    ...
    <button class="bo-segment-btn ${activeTid === t.id ? "active" : ""}"
      onclick="${onchangeFn}('${t.id}')">${t.name}</button>`,
    ).join("")}
  </div>`;
}
...
function renderStep1() {
  ...
  const isOwner =
    (boCurrentPersona.ownedAccounts || []).length > 0 ||
    (boCurrentPersona.ownedAccounts || [])[0] === "*";
  const vId = boCurrentPersona.vorgId || "";
  const vLabel = vId.includes("RND")
    ? "🔬 R&D 예산 전용 계정 보기"
    : vId === "SYSTEM"
      ? "🌐 전체 계정 조회"
      : "📋 일반교육 예산 전용 계정 보기";
        ...
        ${allAccounts
          .map((a) => {
            const planToggle = a.isSystem
              ...
              : bmToggle(
                  a.planRequired,
                  `s1ToggleField('${a.code}','planRequired')`,
                  "blue",
                ) +
            ...
            const carryToggle = a.isSystem
              ...
              : bmToggle(
                  a.carryover,
                  `s1ToggleField('${a.code}','carryover')`,
                  "green",
                );
...
function s1ToggleField(code, field) {
  ...
  const el =
    document.getElementById("ba-content") ||
    document.getElementById("bm-content");
...
function renderStep2OrgMap() {
  ...
  const allGroups = [
    ...
    ...virtualOrgState.rnd.centers.map((c) => ({
      id: c.id,
      name: c.name,
      type: "R&D센터",
      icon: "🔬",
    ...
    <div class="bo-split-right">
      ...
      <div style="display:flex;flex-direction:column;gap:8px">
        ${allGroups
          ...
          : '<div style="padding:24px;text-align:center;color:#9CA3AF">좌측에서 계정을 선택하세요</div>'
      }
    </div>
  </div>
...
function _s2GetGroups(tid) {
  const all = [];
  virtualOrgState.general.hqs.forEach((h) =>
    all.push({
      id: h.id,
      name: h.name,
      type: "일반본부",
  ...
  virtualOrgState.rnd.centers.forEach((c) =>
    all.push({
      id: c.id,
      name: c.name,
      type: "R&D센터",
...
<div>
  ...
  <div class="bo-split">
    ...
    <div class="bo-split-right">
      ...
      ${
        allForms.length === 0
          ? `<div style="text-align:center;padding:32px;color:#9CA3AF">Step3에서 양식을 먼저 생성해 주세요.</div>`
          : `
...
function renderStep3() {
  ...
  const ruleCards = Object.keys(accGrouped)
    .map((code) => {
      const acc = accounts.find((a) => a.code === code);
      if (!acc) return "";
      const accCards = accGrouped[code]
        .map((r) => {
          const tpl = VIRTUAL_EDU_ORGS.find((t) => t.id === r.templateId) || {
            name: r.templateId || "전체(미지정)",
          };
          const applyForm = r.formId
            ? FORM_MASTER.find((f) => f.id === r.formId) || { name: r.formId }
            : null;
          const planForm = r.planFormId
            ? FORM_MASTER.find((f) => f.id === r.planFormId)
            : null;
          const resultForm = r.resultFormId
            ? FORM_MASTER.find((f) => f.id === r.resultFormId)
            : null;
          ...
          const ltHtml =
            r.learningTypes && r.learningTypes.length
              ? r.learningTypes
                  .map(
                    (lt) =>
                  ...
                  .join("")
              : '<span style="font-size:11px;color:#9CA3AF">모든 유형</span>';
        ...
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px">
          <div style="flex:1">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;flex-wrap:wrap">
              ${flowBadge}
              <span class="bo-rule-label if-label" style="background:#FCE7F3;color:#BE185D">제도그룹</span>
...
function _s3ModalBody(rule) {
  ...
  const ltHtml = LEARNING_TYPES.map(
    (cat) => `
  ...
  const flowOptions = [
    {
      val: "plan-apply-result",
      label: "📋 계획 ➡️ 신청 ➡️ 결과 (3단계)",
      desc: "R&D 예산 등 사전 계획 필수 계정에 적용",
    },
    {
      val: "apply-result",
      label: "📄 신청 ➡️ 결과 (2단계)",
      desc: "일반 참가/운영계정 등 계획 불필요 계정에 적용",
    },
    {
      val: "result-only",
      label: "📝 결과 단독 (1단계)",
      desc: "무예산/자비 학습 이력 등록에만 사용",
    },
  ];
...
function s3UpdateModalDynamic() {
  ...
  if (planDiv)
    planDiv.style.display =
      isPlanReq || flow === "plan-apply-result" ? "block" : "none";
  if (applyDiv)
    applyDiv.style.display = flow !== "result-only" ? "block" : "none";
}

function s3SelectFlow(val) {
  document
    .querySelectorAll('input[name="s3-flow"]')
...
function openAddRuleModal() {
  ...
  const m = document.getElementById("s3-modal");
...
function s3SaveRule() {
  ...
  const fid =
    applyDivEl && applyDivEl.style.display !== "none"
  ...
  if (!acc || !tpl) {
    alert("예산 계정과 대상 조직 제도그룹을 선택해주세요.");
    return;
  }
  if (processFlow !== "result-only" && !fid) {
    alert("신청 양식을 선택해주세요.");
    return;
  }
  if (processFlow === "result-only" && !resultFormId) {
    alert("결과 양식을 선택해주세요.");
    return;
  }
  if (_s3EditingRuleId) {
    ...
    if (r) {
      ...
      r.templateId = tpl;
      r.formId = fid;
      r.planFormId = planFormId;
      r.multiPlanAllowed = multiPlanAllowed;
      r.learningTypes = learningTypes;
      r.processFlow = processFlow;
      r.resultFormId = resultFormId;
    }
  } else {
    FORM_BUDGET_RULES.push({
      id: "R" + Date.now(),
      tenantId: boCurrentPersona.tenantId || _s3Tenant,
      accountCode: acc,
      templateId: tpl,
      formId: fid,
      planFormId,
