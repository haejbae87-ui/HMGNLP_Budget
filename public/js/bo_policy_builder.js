  ...
  <div style="border:2px solid ${stageColor[s]}50;border-radius:14px;overflow:hidden">
    ...
    <div style="padding:16px;background:white;display:grid;gap:18px">
      ...
      ${(() => {
          ...
          const modes = [
            { key: 'none', label: '검토 없음', desc: '결재 승인 후 별도 검토 없이 처리', icon: '⏭️', color: '#9CA3AF' },
            { key: 'admin_only', label: '총괄담당자 최종검토', desc: '총괄담당자가 최종 검토 후 완료 처리', icon: '🏛️', color: '#1D4ED8' },
            { key: 'manager_only', label: '운영담당자 최종검토', desc: '운영담당자가 최종 검토 후 완료 처리', icon: '👤', color: '#D97706' },
            { key: 'both', label: '운영담당자 → 총괄담당자', desc: '1차 운영담당자, 2차 총괄담당자 순서로 검토', icon: '👤→🏛️', color: '#059669' },
          ];
          return '<div style="border-top:1px solid #F3F4F6;padding-top:16px"><label style="font-size:12px;font-weight:800;color:#374151;display:block;margin-bottom:10px">🔍 결재 후 검토자</label><div style="display:grid;gap:8px">' + modes.map(m => {
            const sel = reviewMode === m.key;
            return '<label style="display:flex;align-items:center;gap:10px;padding:12px 14px;border-radius:10px;border:2px solid '+(sel ? m.color : '#E5E7EB')+';background:'+(sel ? m.color+'10' : 'white')+';cursor:pointer;transition:all .15s" onclick="_policyWizardData.approvalConfig[\''+s+'\'].reviewMode=\''+m.key+'\';renderPolicyWizard()"><input type="radio" '+(sel?'checked':'')+' style="margin:0;accent-color:'+m.color+'"><span style="font-size:14px">'+m.icon+'</span><div style="flex:1"><div style="font-size:12px;font-weight:800;color:'+(sel ? m.color : '#374151')+'">'+m.label+'</div><div style="font-size:10px;color:#6B7280;margin-top:1px">'+m.desc+'</div></div></label>';
          }).join('') + '</div></div>';
        })()}
    </div>
  </div>`;
