...
// LNB 독립 진입점 래퍼
function renderFormNoticeMenu() { renderAnnouncementMgmt(); }
...
function renderAnnouncementMgmt() {
  ...
  const tenantName = TENANTS.find(t => t.id === _annTenant)?.name || _annTenant;
...
<div class="bo-fade">
  ...
  <div class="bo-card" style="padding:16px 20px;margin-bottom:20px;background:linear-gradient(135deg,#F0FDF4,#EFF6FF);border-color:#A7F3D0">
    ...
    ].map(([icon, label, color, bg, border], i, arr) => `
  ...
  <div style="display:flex;gap:20px">
    ...
    <div style="width:220px;flex-shrink:0">
      <div class="bo-card" style="overflow:hidden">
        ...
        ${_annGetForms().map(f => {
      const cnt = FORM_ANNOUNCEMENTS.filter(a => a.tenantId === _annTenant && a.formId === f.id && a.active).length;
...
function _renderAnnList() {
  ...
  const anns = FORM_ANNOUNCEMENTS.filter(a => a.tenantId === _annTenant && a.formId === _annSelForm);
...
function _annModalBody(ann) {
  const formType = _annGetForms().find(f => f.id === _annSelForm);
  const fi = formType?.icon || (formType?.type === 'plan' ? '📋' : '📄');
  const inp = (id, ph, val = '', lines = 1) => lines > 1
...
function annSave() {
  ...
  const orig = _annEditId ? FORM_ANNOUNCEMENTS.find(a => a.id === _annEditId)?.attachments || [] : [];
