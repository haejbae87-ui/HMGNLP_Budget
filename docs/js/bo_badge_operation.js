...
async function renderBadgeOperation() {
  ...
  container.innerHTML = `
    ...
    <div style="display:flex;gap:4px;border-bottom:1px solid var(--border);margin-bottom:24px">
      <button id="tab-requests" onclick="switchOpTab('requests')" style="padding:12px 20px;background:#fff;border:1px solid var(--border);border-bottom:none;border-radius:8px 8px 0 0;font-size:14px;font-weight:800;color:var(--brand);margin-bottom:-1px;position:relative;z-index:2">
        수동 심사 요청 
      </button>
      <button id="tab-tracker" onclick="switchOpTab('tracker')" style="padding:12px 20px;background:#f8fafc;border:1px solid transparent;border-bottom:none;border-radius:8px 8px 0 0;font-size:14px;font-weight:700;color:var(--text-sub)">
        학습자 현황 트래커 
      </button>
    </div>
    ...
    <div id="op-content-tracker" style="display:none">
      <div style="background:#fff;border:1px solid var(--border);border-radius:12px;padding:24px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
          ...
          <div style="display:flex;gap:8px">
            <select id="filter-status" onchange="loadProgressData()" style="padding:6px 12px;border:1px solid var(--border);border-radius:6px;font-size:12px">
...
function switchOpTab(tabId) {
  if (tabId === 'requests') {
    document.getElementById('tab-requests').style.cssText = 'padding:12px 20px;background:#fff;border:1px solid var(--border);border-bottom:rgba(255,255,255,1);border-radius:8px 8px 0 0;font-size:14px;font-weight:800;color:var(--brand);margin-bottom:-1px;position:relative;z-index:2';
    document.getElementById('tab-tracker').style.cssText = 'padding:12px 20px;background:#f8fafc;border:1px solid transparent;border-bottom:none;border-radius:8px 8px 0 0;font-size:14px;font-weight:700;color:var(--text-sub)';
    document.getElementById('op-content-requests').style.display = 'block';
    document.getElementById('op-content-tracker').style.display = 'none';
  } else {
    document.getElementById('tab-tracker').style.cssText = 'padding:12px 20px;background:#fff;border:1px solid var(--border);border-bottom:rgba(255,255,255,1);border-radius:8px 8px 0 0;font-size:14px;font-weight:800;color:var(--brand);margin-bottom:-1px;position:relative;z-index:2';
    document.getElementById('tab-requests').style.cssText = 'padding:12px 20px;background:#f8fafc;border:1px solid transparent;border-bottom:none;border-radius:8px 8px 0 0;font-size:14px;font-weight:700;color:var(--text-sub)';
    document.getElementById('op-content-tracker').style.display = 'block';
    document.getElementById('op-content-requests').style.display = 'none';
  }
}

async function loadRequestsData() {
  try {
    const { data: reqs, error } = await window._supabase
      .from('badge_award_requests')
      .select('*, badges(name, allow_manual_award)')
    ...
    document.getElementById('req-body').innerHTML = `<tr><td colspan="6" style="color:red">조회 실패</td></tr>`;
  }
}
...
function renderReqs() {
  ...
  tbody.innerHTML = requestsData.map(r => {
    ...
    if(r.status === 'APPROVED') stat = `<span style="color:#10b981;font-weight:700">승인됨</span>`;
    if(r.status === 'REJECTED') stat = `<span style="color:#ef4444;font-weight:700">반려됨</span>`;
    ...
    return `
      <tr style="border-bottom:1px solid #f3f4f6">
        ...
        <td style="padding:14px;color:var(--brand);font-weight:700">${r.badges?.name || 'Unknown'}</td>
        <td style="padding:14px"><a href="#" style="color:#3b82f6;text-decoration:none">첨부파일(zip)</a></td>
...
async function approveRequest(reqId, userId, badgeId) {
  ...
  // 1. user_badges upsert (ACTIVATE)
  // 2. badge_award_requests update (APPROVED)
  try {
    const {data: badgeArr} = await window._supabase.from('badges').select('valid_months').eq('id', badgeId);
    let valid_months = badgeArr?.[0]?.valid_months || null;
    let expiresAt = null;
    if(valid_months) {
      let d = new Date();
...
async function loadProgressData() {
  ...
  try {
    let q = window._supabase.from('user_badges').select('*, badges(name, valid_months)').order('created_at', { ascending: false });
    if(flt === 'EXPIRED_SOON') {
       // logic for 30 days remaining
    ...
    document.getElementById('prog-body').innerHTML = `<tr><td colspan="6" style="color:red">조회 실패</td></tr>`;
  }
}
...
function renderProgs() {
  ...
  tbody.innerHTML = progressData.map(p => {
    let statStyle = 'color:#64748b';
    let statText = p.status;
    if(p.status === 'COURSE_COMPLETED') { statStyle='color:#3b82f6;font-weight:700'; statText='시험 대기자'; }
    if(p.status === 'ACTIVE') { statStyle='color:#10b981;font-weight:700'; statText='보유 중'; }
    if(p.status === 'EXPIRED') { statStyle='color:#ef4444;font-weight:700'; statText='강등/만료됨'; }

    return `
      <tr style="border-bottom:1px solid #f3f4f6">
        <td style="padding:14px;color:var(--brand);font-weight:800">${p.badges?.name || 'Unknown'}</td>
        ...
        <td style="padding:14px;text-align:right">
          ...
          ${document.getElementById('filter-status').value === 'EXPIRED_SOON' ? `<button style="padding:4px 8px;background:#f59e0b;color:#fff;border:none;border-radius:4px;font-size:12px;cursor:pointer">갱신 독려 발송</button>` : ''}
...
async function testForceActivate(uid, bid) {
  if(!confirm('이 사용자를 시험/과정 이수와 무관하게 수동으로 패스/뱃지 발급하시겠습니까?')) return;
  alert('직권 강제 발급 기능이 데모용으로 설정되었습니다. 실제 발급로직 구현 필요.');
}

async function forceAwardBadge() {
  const uid = prompt("뱃지를 직권으로 부여할 사용자 (글로벌 user_id) 를 입력하세요:");
  if(!uid) return;
  alert('해당 기능은 뱃지 목록을 보여주는 모달로 구현되어야 합니다. (아키텍처 상 지원 가능하도록 DB 스키마 완비)');
}
