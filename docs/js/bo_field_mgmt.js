...
async function renderFieldMgmt() {
  ...
  if (!_fmTenantId) {
    _fmTenantId = isPlatform
      ? tenants[0]?.id || "HMC"
      : boCurrentPersona.tenantId || "HMC";
  }

  const tenantName =
    tenants.find((t) => t.id === _fmTenantId)?.name || _fmTenantId;
  ...
  try {
    ...
    if (sb) {
      const { data } = await sb
        ...
        .or(
          "service_type.ilike.%edu_support%,purpose.eq.edu_support,purpose.eq.교육지원",
        );
      if (data && data.length) {
  ...
  const tplSelectHtml =
    _fmLoadedTpls.length > 1
      ...
      : curTpl
        ? `<span style="font-size:12px;font-weight:700;background:#EFF6FF;color:#1D4ED8;padding:6px 12px;border-radius:8px">📋 ${curTpl.name}</span>`
        : "";

  // -- 그룹 목록 --
  const groups = curTpl ? curTpl.tree?.hqs || curTpl.tree?.centers || [] : [];
  const allGroups = groups.map((g, gi) => ({
    ...g,
    _tplId: curTpl?.id,
    _tplName: curTpl?.name,
    _gi: gi,
  }));

  const groupsHtml = allGroups.length
    ? allGroups
        .map((g) => {
          const gi = g._gi;
...
window._fmOpenManagerModal = async function (tplId, groupIdx) {
  ...
  document.getElementById("fm-mgr-title").textContent =
    `👤 ${gName} 담당자 설정`;
  document.getElementById("fm-mgr-subtitle").textContent =
    `${_fmTenantId} 사용자 중에서 선택`;
  ...
  try {
    ...
    if (sb) {
      const { data } = await sb
        .from("users")
        .select("id,name,emp_no,job_type")
        .eq("tenant_id", _fmTenantId);
      users = data || [];
    }
  } catch (e) {}
...
function _fmRenderMgrList(users) {
  ...
  if (!users.length) {
    el.innerHTML =
      '<p style="font-size:11px;color:#94A3B8;text-align:center;padding:12px">사용자 없음</p>';
    return;
  }
...
window._fmAddManager = function (userId, name) {
  ...
  if (!g.managers.some((m) => m.userId === userId))
    g.managers.push({ userId, name });
...
window._fmRemoveManager = function (tplId, groupIdx, userIdOrName) {
  ...
  g.managers = g.managers.filter(
    (m) => m.userId !== userIdOrName && m.name !== userIdOrName,
  );
  renderFieldMgmt();
  _fmExpandGroup(tplId, groupIdx);
};
...
window._fmOpenCoopModal = async function (tplId, groupIdx) {
  ...
  const { g } = _fmFindGroup(tplId, groupIdx);
  const gName = g?.name || g?.label || "";
  document.getElementById("fm-coop-title").textContent =
    `🤝 ${gName} 협조처 추가`;
  document.getElementById("fm-coop-org-search").value = "";
  document.getElementById("fm-coop-selected-info").innerHTML =
    '<span style="color:#9CA3AF;font-size:12px">조직을 선택하면 여기에 표시됩니다</span>';

  // 조직 목록 DB 로드
  const orgList = document.getElementById("fm-coop-org-list");
  orgList.innerHTML =
    '<div style="text-align:center;padding:20px;color:#9CA3AF;font-size:12px">조직 로딩 중...</div>';
...
function _fmRenderCoopOrgList(orgs) {
  ...
  if (!orgs.length) {
    el.innerHTML =
      '<div style="text-align:center;padding:20px;color:#9CA3AF;font-size:12px">조직 없음</div>';
    return;
  }
  const typeStyle = {
    headquarters: "#1E40AF",
...
window._fmSaveCoop = function () {
  ...
  g.coopTeams.push({
    team: _fmCoopOrgSel.name,
    orgId: _fmCoopOrgSel.id,
    type: coopType,
    required,
  });
