...
function renderPlans() {
  ...
  const floatingActionBar = _selectedPlans.length > 0 ? `
    <div style="position:fixed;bottom:32px;left:50%;transform:translateX(-50%);background:#002C5F;padding:16px 24px;border-radius:16px;box-shadow:0 10px 25px rgba(0,44,95,.4);display:flex;align-items:center;gap:24px;z-index:9999;color:white;width:max-content;">
      <div>
        <div style="font-size:14px;font-weight:900;">📤 선택 건 일괄 상신</div>
        <div style="font-size:12px;color:#93C5FD;margin-top:2px">${_selectedPlans.length}건 선택됨 / ${Number(filteredPlans.filter(p=>_selectedPlans.includes(p.id)).reduce((sum,p)=>sum+(p.amount||0),0)).toLocaleString()}원</div>
      </div>
...
function _renderPlanDetailView(plan) {
  ...
  // fo_plans_actions.js에 정의된 단일 뷰 컴포넌트를 공유
  if (typeof foRenderPlanUnifiedView === 'function') {
    return foRenderPlanUnifiedView(plan, { mode: 'detail' });
  }

  // 폴백: foRenderPlanUnifiedView 미로드 시 기본 뷰
  const st = plan.status || "pending";
  const amount = Number(plan.amount || plan.planAmount || 0);
  return `
  <div class="max-w-3xl mx-auto">
    <div style="margin-bottom:16px">
      <button onclick="_viewingPlanDetail=null;renderPlans()" style="padding:8px 16px;border-radius:10px;border:1.5px solid #E5E7EB;background:white;font-size:12px;font-weight:700;color:#6B7280;cursor:pointer">← 목록으로</button>
    </div>
    <div style="border-radius:16px;overflow:hidden;border:1.5px solid #E5E7EB;background:white;box-shadow:0 4px 20px rgba(0,0,0,.06)">
      <div style="padding:24px 28px;background:linear-gradient(135deg,#002C5F,#0369A1);color:white">
        ...
        <p style="margin:6px 0 0;font-size:12px;opacity:.8">${plan.applicant_name || ""} · ${plan.dept || ""}</p>
      </div>
      <div style="padding:24px 28px;background:#F9FAFB">
        ${typeof window.foRenderStandardReadOnlyForm === 'function' ? window.foRenderStandardReadOnlyForm({...plan, amount, accountCode: plan.account_code || plan.account || ''}, 'FO') : '<p>로딩 중...</p>'}
      </div>
...
// ★ 상세 뷰에서 직접 상신하기 (saved 상태 계획을 확인 화면 없이 바로 planState로 로드 후 confirmPlan)
async function submitFromDetail(planId) {
  const sb = typeof getSB === 'function' ? getSB() : null;
  if (!sb) { alert('DB 연결 실패'); return; }
  try {
    const { data: plan, error } = await sb.from('plans').select('*').eq('id', planId).single();
    if (error || !plan) throw new Error('계획 조회 실패');
    ...
    planState = {
      ...
      plan_type: plan.plan_type || 'operation',
      fiscal_year: plan.fiscal_year,
      eduType: plan.edu_type || d.eduType,
      eduSubType: d.eduSubType,
      purpose: d.purpose ? { id: d.purpose } : null,
      purpose_text: d.purpose_text || '',
      calcGrounds: d.calcGrounds || [],
      locations: d.locations || [],
      budgetId: d.budgetId || null,
      contextAccountCode: plan.account_code,
      accountCode: plan.account_code,
      isOverseas: plan.is_overseas || false,
      overseasCountry: plan.overseas_country || null,
      venueType: plan.venue_type || 'internal',
      plannedRounds: plan.planned_rounds || 1,
      plannedDays: plan.planned_days || null,
      expectedBenefit: plan.expected_benefit || '',
      educationFormat: plan.education_format || '',
      content: d.content || '',
      notes: d.notes || null,
      institution: d.institution || null,
      policyId: plan.policy_id || null,
      formTemplate: d._form_snapshot ? { id: d._form_snapshot.id, inlineFields: null } : null,
      confirmMode: true,
    };
    _viewingPlanDetail = null;
    renderPlans();
  } catch (err) {
    alert('상신 준비 실패: ' + err.message);
  }
}
window.submitFromDetail = submitFromDetail;
