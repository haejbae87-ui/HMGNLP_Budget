...
| `pending` | 결재대기 | 결재대기(Active) | **[조회/확인 화면]** ↩️ 회수하기, 📱 복제 |
| `approved`| 승인완료 | 승인완료(Done) | **[조회/확인 화면]** ▶ 이 계획으로 교육신청, 📉 배정액 축소, 📱 복제 |
| `rejected`| 반려 | 반려(Failed) | **[조회/확인 화면]** ✏️ 수정 (복제 후 재상신 유도) |

> **수정 진입 시 정책 (A안 채택)**: `saved`, `rejected` 상태의 문서를 [수정]할 경우 다시 작성 폼 화면으로 진입하며, 이때 임시저장 버튼이 부활하여 중간 저장(Draft)이 가능해집니다. 이를 통해 필수값 누락 상태라도 중간 작업본 유실을 방지합니다.
