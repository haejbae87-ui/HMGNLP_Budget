...
다건 상신 시 결재선 및 그룹핑 규칙 (v1.4 확정):
- 금액 합산 → 합산 기준으로 결재 구간 결정
- **[계획 묶음 상신]**: 동일 예산계정(`account_code`) 내에서는 교육유형(`edu_type`) 혼합 가능. (실제 자금 집행이 일어나지 않으므로 복잡한 협조처/결재선 분기 불필요)
- **[신청 묶음 상신]**: 동일 예산계정(`account_code`) + **동일 교육유형(`edu_type`)**의 건만 묶음 가능. (교육유형별로 금액에 따른 결재선 및 협조처가 달라지므로 혼합 시 결재 라우팅 오류 발생 위험 차단)

> **Q-SUB1 (v1.4 수정)**: ✅ **확정** — 계획 상신 시에는 **동일 계정** 제약만 걸고, 신청 상신 시에는 **동일 계정 + 동일 교육유형** 제약을 추가로 적용한다. 조건 불일치 건 선택 시 UI에서 경고 후 분리 상신 유도.
