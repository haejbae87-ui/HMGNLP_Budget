...
### 2026-05-16 — 예산 배분 관리 UI 개선

- [x] **예산 요약 카드 리디자인 — 2-Section + 적층 바(Stacked Bar) 시각화**
  - `bo_alloc_drilldown.js`: 기존 산술식(기초+추가=총예산−배분=미배분)을 좌/우 2-Section 레이아웃으로 교체
  - 왼쪽: 총 예산 구성 (기초 배정, 추가 배정, 총 예산)
  - 오른쪽: 예산 사용 현황 — 🔴집행확정 / 🟠약정홀딩 / 🟢가용예산 / ⚪미배분 4단계 적층 바
  - 배분율/소진율 표시를 사전 계산 변수로 정리 (인라인 IIFE 중복 제거)
- [x] **중복 계정 선택 탭(acctTabs) 제거**
  - `bo_alloc_drilldown.js`: 상단 데이터 범위 필터의 '계정' 드롭다운과 중복되는 acctTabs 카드 UI 제거 (~25줄 삭제)
  - 계정 전환을 상단 필터로 통일하여 UX 단순화 및 상태 관리 일원화
