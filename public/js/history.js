  ...
  <div class="card overflow-hidden">
    <table class="w-full text-sm">
      ...
      <tfoot class="bg-blue-50 border-t-2 border-brand">
        <tr>
          <td colspan="3" class="px-6 py-4 text-xs font-black text-gray-500 uppercase">합계 (${MOCK_HISTORY.length}건)</td>
          <td class="px-4 py-4 text-right font-black text-brand">${MOCK_HISTORY.reduce((s, h) => s + h.hours, 0)}H</td>
          <td class="px-4 py-4 text-right font-black text-brand">${fmt(MOCK_HISTORY.reduce((s, h) => s + h.amount, 0))}원</td>
          <td colspan="2"></td>
        </tr>
      </tfoot>
    </table>
