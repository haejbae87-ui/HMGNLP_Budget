...
async function _renderResultVorgHub() {
  ...
  if (sb && vorgIds.length > 0) {
    try {
      const { data } = await sb.from('virtual_org_templates').select('id, name').in('id', vorgIds);
      if (data && data.length > 0) {
        let policyMap = {};
        try {
          const { data: policies } = await sb.from('budget_account_org_policy').select('vorg_template_id, budget_account_id').in('vorg_template_id', vorgIds);
          if (policies && policies.length > 0) {
            const acctIds = [...new Set(policies.map(p => p.budget_account_id))];
            const { data: accts } = await sb.from('budget_accounts').select('id, code').in('id', acctIds);
            const acctCodeMap = {};
        ...
        } catch(e) { console.warn('[ResultVorgHub] policy fetch failed:', e.message); }
        fetchedVorgs = data.map(row => ({ id: row.id, name: row.name || row.id, ownedAccounts: policyMap[row.id] || [] }));
      }
