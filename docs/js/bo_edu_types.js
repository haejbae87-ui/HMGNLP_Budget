...
function _renderEduTypeMgmtUI() {
  ...
  const filtered = _eduMgr.filter === 'all'
    ? _eduMgr.purposes
  ...
  <div style="display:flex;gap:4px;background:#F3F4F6;padding:4px;border-radius:12px;width:fit-content;margin-bottom:20px">
    ${[['all','전체'],['learner','학습자'],['admin','교육담당자']].map(([v,l]) => `
    <button onclick="_eduMgr.filter='${v}';_renderEduTypeMgmtUI()" style="
      ...
      box-shadow:${_eduMgr.filter===v?'0 1px 4px rgba(0,0,0,.1)':'none'}">${l}</button>`).join('')}
  </div>`;
...
function _renderPurposeNode(p) {
  ...
  const audienceBadge = p.audience === 'learner'
  ...
  return `
<div style="border:1.5px solid #E5E7EB;border-radius:14px;overflow:hidden;opacity:${p.is_active?1:.6}">
  <!-- 목적군 헤더 -->
...
async function _eduSavePurpose(existingId) {
  ...
  if (!label) { alert('레이블을 입력하세요.'); return; }

  try {
    if (existingId) {
      // 업데이트
      const { error } = await sb.from('edu_purpose_groups').update({
        label, icon: document.getElementById('em-icon')?.value || '📚',
      ...
      const { error } = await sb.from('edu_purpose_groups').insert({
        id, label, audience: document.getElementById('em-audience')?.value || 'learner',
        icon: document.getElementById('em-icon')?.value || '📚',
        description: document.getElementById('em-desc')?.value || '',
        sort_order: _eduMgr.purposes.length + 1, is_active: true
      });
      if (error) throw error;
    }
    _eduCloseModal();
...
async function _eduSaveGroup(existingId, purposeId) {
  ...
  try {
    if (existingId) {
      const { error } = await sb.from('edu_type_groups').update({ label }).eq('id', existingId);
      if (error) throw error;
    } else {
      const id = document.getElementById('em-id')?.value?.trim();
      if (!id) { alert('ID를 입력하세요.'); return; }
      const { error } = await sb.from('edu_type_groups').insert({
        id, label, purpose_id: purposeId,
        sort_order: _eduMgr.groups.filter(g => g.purpose_id === purposeId).length + 1, is_active: true
      });
      if (error) throw error;
    }
    _eduCloseModal(); renderEduTypeMgmt();
  } catch (e) { alert('저장 실패: ' + e.message); }
}
...
async function _eduSaveItem(existingId, groupId, purposeId) {
  ...
  try {
    if (existingId) {
      const { error } = await sb.from('edu_type_items').update({ label }).eq('id', existingId);
      if (error) throw error;
    } else {
      const id = document.getElementById('em-id')?.value?.trim();
      if (!id) { alert('ID를 입력하세요.'); return; }
      const { error } = await sb.from('edu_type_items').insert({
        id, label, group_id: groupId, purpose_id: purposeId,
        sort_order: _eduMgr.items.filter(x => x.group_id === groupId).length + 1, is_active: true
      });
      if (error) throw error;
    }
    _eduCloseModal(); renderEduTypeMgmt();
...
async function _eduToggleActive(type, id, newVal) {
  const sb = getSB(); if (!sb) return;
  const tableMap = { purpose:'edu_purpose_groups', group:'edu_type_groups', item:'edu_type_items' };
  const table = tableMap[type];
  if (!table) return;
  try {
    const { error } = await sb.from(table).update({ is_active: newVal }).eq('id', id);
    if (error) throw error;
    renderEduTypeMgmt();
