// bo_badge_mgmt.js — 뱃지 서비스 정책 (Step 위저드: 선행조건→취득조건→갱신조건)

let mgmtBadgeGroups = [], allBadges = [], _bmAllTenants = [], _bmVorgTemplates = [];
let _bmFilterTenantId = '', _bmFilterVorgId = '', _bmFilterGroupId = '';
let _bmWizardBadgeId = null, _bmWizardBadge = null, _bmWizardStep = 1;
let _bmConditionNodes = [], _bmReturnGroupId = null;

// ── 메인 목록 ──────────────────────────────────────────────────────────────
async function renderBadgeMgmt() {
  _bmWizardBadgeId = null; _bmWizardStep = 1;
  ...
  document.getElementById('bo-content').innerHTML = `
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
      <div><h1 class="bo-page-title">🏷️ 뱃지 서비스 정책</h1>
...
async function _bmLoadTenants(isSA, myT) {
  ...
  _bmAllTenants = data||[];
  const s = document.getElementById('bm-filter-tenant'); if(!s) return;
  s.innerHTML = `<option value="">전체 회사</option>`+_bmAllTenants.map(t=>`<option value="${t.id}"${t.id===myT?' selected':''}>${t.name}(${t.id})</option>`).join('');
  _bmFilterTenantId=myT; s.value=myT;
}
async function _bmLoadVorgs(tenantId) {
  ...
  const {data}=await q; _bmVorgTemplates=data||[];
  const s=document.getElementById('bm-filter-vorg'); if(!s) return;
  s.innerHTML=`<option value="">전체 가상조직</option>`+_bmVorgTemplates.map(v=>`<option value="${v.id}">${v.name}</option>`).join('');
  _bmFilterVorgId='';
  const g=document.getElementById('bm-filter-group'); if(g) g.innerHTML=`<option value="">전체 그룹</option>`; _bmFilterGroupId='';
}
async function _bmLoadGroups(vorgId) {
  ...
  const {data}=await q.order('name'); mgmtBadgeGroups=data||[];
  const g=document.getElementById('bm-filter-group'); if(!g) return;
  g.innerHTML=`<option value="">전체 그룹</option>`+mgmtBadgeGroups.map(g=>`<option value="${g.id}">${g.name}</option>`).join('');
  _bmFilterGroupId='';
}
async function onBmTenantChange() {
  const s=document.getElementById('bm-filter-tenant'); _bmFilterTenantId=s?s.value:''; _bmFilterVorgId=''; _bmFilterGroupId='';
  await _bmLoadVorgs(_bmFilterTenantId); await loadBadgeMgmtData();
}
async function onBmVorgChange() {
  const s=document.getElementById('bm-filter-vorg'); _bmFilterVorgId=s?s.value:''; _bmFilterGroupId='';
  await _bmLoadGroups(_bmFilterVorgId); await loadBadgeMgmtData();
}

async function loadBadgeMgmtData() {
  const tenantId=_bmFilterTenantId||boCurrentPersona?.tenantId||'HMC';
  const tb=document.getElementById('badges-body');
  if(tb) tb.innerHTML=`<tr><td colspan="7" style="text-align:center;padding:20px;color:#9CA3AF">불러오는 중...</td></tr>`;
  try {
    if(!mgmtBadgeGroups.length) {
      ...
      const {data:gd}=await gq; mgmtBadgeGroups=gd||[];
    }
    let gIds=_bmFilterGroupId?[_bmFilterGroupId]:mgmtBadgeGroups.map(g=>g.id);
    if(!gIds.length){allBadges=[];renderBadgesList();return;}
    const {data,error}=await _sb().from('badges').select('*').in('group_id',gIds).order('sort_order',{ascending:true});
    if(error) throw error;
    allBadges=data||[]; renderBadgesList();
  } catch(e) {
    const tb=document.getElementById('badges-body');
    if(tb) tb.innerHTML=`<tr><td colspan="7" style="text-align:center;padding:20px;color:#EF4444">⚠️ ${e.message}</td></tr>`;
  }
}

function renderBadgesList() {
  const tb=document.getElementById('badges-body'); if(!tb) return;
  if(!allBadges.length){tb.innerHTML=`<tr><td colspan="7" style="text-align:center;padding:32px;color:#9CA3AF">뱃지가 없습니다. 뱃지 그룹 관리에서 뱃지를 먼저 생성하세요.</td></tr>`;return;}
  tb.innerHTML=allBadges.map(b=>{
    const gName=mgmtBadgeGroups.find(g=>g.id===b.group_id)?.name||'-';
    const hasPrereq=b.prerequisite_badge_id||(b.equivalent_badge_ids||[]).length;
    const hasCond=(b.condition_rules?.nodes||[]).length>0;
    const hasRenew=b.renewal_rules&&Object.keys(b.renewal_rules).length>0;
    const badge=(v,label)=>v?`<span style="font-size:10px;padding:2px 8px;border-radius:10px;background:#ECFDF5;color:#059669;font-weight:700">✅ ${label}</span>`:`<span style="font-size:10px;padding:2px 8px;border-radius:10px;background:#FEF2F2;color:#DC2626;font-weight:700">미설정</span>`;
...
async function openBadgePolicy(badgeId, returnGroupId=null) {
  ...
  if(!b){ alert('뱃지 정보를 찾을 수 없습니다.'); return; }
  _bmWizardBadgeId=badgeId; _bmWizardBadge=JSON.parse(JSON.stringify(b));
  _bmWizardStep=1; _bmReturnGroupId=returnGroupId;
  _bmConditionNodes=Array.isArray(b.condition_rules?.nodes)?JSON.parse(JSON.stringify(b.condition_rules.nodes)):[];
  _bmRenderWizard();
}
...
function _bmRenderWizard() {
  ...
  const gName=mgmtBadgeGroups.find(g=>g.id===b.group_id)?.name||'';
  const steps=[{n:1,label:'선행 조건'},{n:2,label:'취득 조건'},{n:3,label:'갱신 조건'}];
  const stepNav=steps.map(s=>{
    const active=s.n===_bmWizardStep, done=s.n<_bmWizardStep;
...
function _bmRenderStep1() {
  const b=_bmWizardBadge;
  const prereqOpts=allBadges.filter(x=>x.id!==_bmWizardBadgeId).map(x=>`<option value="${x.id}"${b.prerequisite_badge_id===x.id?' selected':''}>${x.name}</option>`).join('');
...
function _bmRenderStep2() {
  ...
  const nodesHtml=_bmConditionNodes.length===0
...
function _bmStepButtons(step) {
  const prev=step>1?`<button onclick="_bmGoToStep(${step-1})" style="padding:10px 20px;background:#F3F4F6;color:#6B7280;border:none;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer">← 이전</button>`:'';
  const next=step<3?`<button onclick="_bmGoToStep(${step+1})" style="padding:10px 24px;background:linear-gradient(135deg,#1D4ED8,#2563EB);color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:800;cursor:pointer">다음 →</button>`
...
async function _bmGoToStep(n) {
  // 현재 스텝 데이터 수집
  if(_bmWizardStep===1) { _bmWizardBadge.prerequisite_badge_id=document.getElementById('bm-prereq')?.value||null; try{_bmWizardBadge.equivalent_badge_ids=JSON.parse(document.getElementById('bm-equivalent')?.value||'[]');}catch{return alert('JSON 형식 오류: 타사 인정 뱃지 ID를 확인해주세요.');} }
  if(_bmWizardStep===2) { const op=document.getElementById('bmd-operator')?.value||'AND'; _bmWizardBadge.condition_rules={operator:op,nodes:_bmConditionNodes}; }
  if(_bmWizardStep===3) { const isSame=document.querySelector('input[name="renewal-type"]:checked')?.value==='same'; _bmWizardBadge.renewal_rules=isSame?{}:(() => { try{return JSON.parse(document.getElementById('bm-renewal')?.value||'{}');}catch{alert('갱신 조건 JSON 오류');return null;} })(); if(_bmWizardBadge.renewal_rules===null) return; }
  _bmWizardStep=n; _bmRenderWizard();
}

async function saveBadgePolicy() {
  // Step3 데이터 수집
  const isSame=document.querySelector('input[name="renewal-type"]:checked')?.value==='same';
  let renewal_rules={};
  if(!isSame){ try{renewal_rules=JSON.parse(document.getElementById('bm-renewal')?.value||'{}');}catch{return alert('갱신 조건 JSON 오류');} }
  const b=_bmWizardBadge;
  const payload={
  ...
  const {error}=await _sb().from('badges').update(payload).eq('id',_bmWizardBadgeId);
...
// ── 레거시 호환 ──────────────────────────────────────────────────────────────
function openBadgeMgmtModal() { renderBadgeMgmt(); }
function openBadgeMgmtDetail(id, gid) { if(id){const b=allBadges.find(x=>x.id===id);if(b){_bmReturnGroupId=gid||null;openBadgePolicy(id,gid);}else renderBadgeMgmt();}else renderBadgeMgmt(); }
function editBadge(id) { openBadgePolicy(id); }
function deleteBadge(id) { if(!confirm('삭제하시겠습니까?'))return; _sb().from('badges').delete().eq('id',id).then(({error})=>{ if(error)alert('삭제 실패: '+error.message); else loadBadgeMgmtData(); }); }
