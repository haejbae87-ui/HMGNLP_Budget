...
const BH_ACTION_COLORS = {
    allocate: '#059669', adjust: '#D97706', freeze: '#7C3AED', use: '#DC2626',
    carryover: '#1D4ED8', release: '#6B7280', transfer_out: '#EF4444',
    transfer_in: '#059669', deactivate: '#9CA3AF', topup: '#0891B2'
};
const BH_ACTION_LABELS = {
    allocate: '배정', adjust: '조정', freeze: '동결', use: '집행',
    carryover: '이월', release: '해제', transfer_out: '이관(출)',
    transfer_in: '이관(입)', deactivate: '비활성화', topup: '추가배정'
};
...
async function renderBudgetHistory() {
    ...
    if (!_bhTenant) _bhTenant = isPlatform ? (tenants[0]?.id || 'HMC') : (boCurrentPersona.tenantId || 'HMC');
    ...
    if (_bhTplId) {
        try {
            const { data } = await sb.from('virtual_org_templates').select('tree_data').eq('id', _bhTplId).single();
    ...
    // 계정 로드
    if (_bhTplId) {
        try { const { data } = await sb.from('budget_accounts').select('id,name,code').eq('virtual_org_template_id', _bhTplId).eq('tenant_id', _bhTenant).eq('active', true); _bhAcctList = data || []; } catch (e) { _bhAcctList = []; }
    } else { _bhAcctList = []; }
...
async function _bhLoadLogs() {
    const sb = typeof _sb === 'function' ? _sb() : null;
    if (!sb || !_bhTplId) { _bhLogs = []; _bhTotalCount = 0; return; }

    // 1. 통장 로드 (필터 적용)
    let bbQ = sb.from('org_budget_bankbooks').select('id,org_name,org_id,account_id,vorg_group_id,status').eq('tenant_id', _bhTenant).eq('template_id', _bhTplId);
    if (_bhGroupId) bbQ = bbQ.eq('vorg_group_id', _bhGroupId);
    if (_bhAccountId) bbQ = bbQ.eq('account_id', _bhAccountId);

    let bankbooks = [];
    try { const { data } = await bbQ; bankbooks = data || []; } catch (e) { bankbooks = []; }
    ...
    if (!bankbooks.length) { _bhLogs = []; _bhTotalCount = 0; return; }

    // 2. allocation 로드
    const bbIds = bankbooks.map(b => b.id);
    let allocs = [];
    try { const { data } = await sb.from('budget_allocations').select('id,bankbook_id').in('bankbook_id', bbIds); allocs = data || []; } catch (e) { allocs = []; }

    _bhAllocationMap = {};
    allocs.forEach(a => { _bhAllocationMap[a.id] = a.bankbook_id; });

    if (!allocs.length) { _bhLogs = []; _bhTotalCount = 0; return; }

    // 3. 로그 로드 (필터 + 페이지네이션)
    const allocIds = allocs.map(a => a.id);
    let logQ = sb.from('budget_allocation_log').select('*', { count: 'exact' }).in('allocation_id', allocIds);
    ...
    logQ = logQ.order('performed_at', { ascending: false }).range(_bhPage * _bhPageSize, (_bhPage + 1) * _bhPageSize - 1);

    try {
        const { data, count } = await logQ;
        _bhLogs = data || [];
        _bhTotalCount = count || 0;
    } catch (e) { _bhLogs = []; _bhTotalCount = 0; }
    ...
    if (_bhSearchText) {
        ...
        _bhLogs = _bhLogs.filter(l => {
            ...
            return (bb?.org_name || '').toLowerCase().includes(q) ||
...
function _bhRender(el, isPlatform, tenants) {
    ...
    const tenantSel = isPlatform ? `<select onchange="_bhTenant=this.value;_bhTplId=null;_bhGroupId=null;_bhAccountId=null;_bhPage=0;renderBudgetHistory()"
    style="border:1.5px solid #E5E7EB;border-radius:8px;padding:6px 10px;font-size:12px;min-width:120px">
    ${tenants.filter(t => t.id !== 'SYSTEM').map(t => `<option value="${t.id}" ${t.id === _bhTenant ? 'selected' : ''}>${t.name}</option>`).join('')}
  </select>` : `<span style="font-size:12px;font-weight:700;color:#374151">${tenants.find(t => t.id === _bhTenant)?.name || _bhTenant}</span>`;

    const tplSel = _bhTplList.length ? `<select onchange="_bhTplId=this.value;_bhGroupId=null;_bhAccountId=null;_bhPage=0;renderBudgetHistory()"
    ...
    const groupSel = _bhGroups.length ? `<select onchange="_bhGroupId=this.value||null;_bhPage=0;renderBudgetHistory()"
    style="border:1.5px solid #E5E7EB;border-radius:8px;padding:6px 10px;font-size:12px;min-width:120px">
    <option value="">전체 그룹</option>
    ${_bhGroups.map(g => `<option value="${g.id}" ${g.id === _bhGroupId ? 'selected' : ''}>${g.name}</option>`).join('')}
  </select>` : '';

    const acctSel = _bhAcctList.length ? `<select onchange="_bhAccountId=this.value||null;_bhPage=0;renderBudgetHistory()"
    style="border:1.5px solid #E5E7EB;border-radius:8px;padding:6px 10px;font-size:12px;min-width:120px">
...
<div class="bo-fade" style="max-width:1200px">
  ...
  <div class="bo-card" style="overflow:hidden">
    <table class="bo-table" style="font-size:11px;width:100%">
      ...
      <tbody>
      ${_bhLogs.length ? _bhLogs.map((l, i) => {
        const bbId = _bhAllocationMap[l.allocation_id];
        const bb = _bhBankbookMap[bbId] || {};
        const c = BH_ACTION_COLORS[l.action] || '#6B7280';
        const ac = Number(l.amount) >= 0 ? '#059669' : '#EF4444';
        const inactive = bb.status === 'inactive' ? ' <span style="font-size:8px;padding:1px 3px;background:#FEE2E2;color:#DC2626;border-radius:3px;font-weight:800">비활성</span>' : '';
    ...
    }).join('') : `<tr><td colspan="8" style="padding:40px;text-align:center;color:#9CA3AF;font-size:12px">
...
function _bhExportCSV() {
    if (!_bhLogs.length) { alert('내보낼 데이터가 없습니다.'); return; }
    const fmt = n => Number(n).toLocaleString();
    const header = ['일시', '조직', '예산계정', '유형', '금액', '이전잔액', '이후잔액', '사유', '처리자'];
