...
let _voHoverTemplateId = null;

// ─── 내부 리렌더 헬퍼 (bo-content 기준) ──────────────────────────────────────
function _voRerender() {
  const el = document.getElementById('bo-content');
  if (el) el.innerHTML = _renderVirtualOrgFull();
}
        ...
        <li style="margin-bottom:16px">
          ...
          <ul class="org-tree" style="padding-left:0">
            ${g.teams.map((t, tIdx) => {
              ...
              const selectAllBtn = '<label style="display:inline-flex;align-items:center;gap:3px;font-size:10px;cursor:pointer;padding:2px 8px;border-radius:5px;font-weight:900;background:'+(allSelected?'#1D4ED8':'#EFF6FF')+';color:'+(allSelected?'white':'#1D4ED8')+';border:1px solid #BFDBFE" '+
                'onclick="voSelectAllJobTypes('+gIdx+','+tIdx+')">'+
                '<input type="checkbox" '+(allSelected?'checked':'')+' style="margin:0;width:10px;height:10px"> 전체</label> ';
              const jtPicker = selectAllBtn + allTypes.map(jt => {
                const sel = jts.includes(jt);
                return '<label style="display:inline-flex;align-items:center;gap:3px;font-size:10px;cursor:pointer;padding:2px 6px;border-radius:5px;background:'+(sel?'#059669':'#F3F4F6')+';color:'+(sel?'white':'#374151')+'" '+
                       'onclick="voToggleJobType('+gIdx+','+tIdx+',&apos;'+jt+'&apos;)">'+
                       '<input type="checkbox" '+(sel?'checked':'')+' style="margin:0;width:10px;height:10px"> '+jt+'</label>';
              }).join(' ');
              return `
...
function voOpenCreateTemplate() {
  document.getElementById('vo-tpl-name').value = '';
  document.getElementById('vo-tpl-type').value = 'general';
...
function voOpenCreateGroup(budgetType) {
  ...
  document.getElementById('vo-group-name').value = '';
  document.getElementById('vo-group-manager').value = '';
  _voEditGroupIdx = null; // 신규 생성
...
function voOpenEditGroup(groupIdx) {
  ...
  document.getElementById('vo-group-create-title').textContent = `"${g.name}" 수정`;
  document.getElementById('vo-group-name').value = g.name;
  document.getElementById('vo-group-manager').value = g.manager || '';
...
function voConfirmCreateGroup() {
  ...
  if (_voEditGroupIdx !== null && _voEditGroupIdx !== undefined) {
    ...
    // 신규 생성
    const newG = { id:'VG'+Date.now(), name, manager:mgr, managerPersonaKey:mgrKey,
...
function voOpenCoopModal(groupIdx) {
  ...
  document.getElementById('vo-coop-team-input').value = '';
  document.getElementById('vo-coop-role-input').value = '';
...
function voOpenAddTeam(groupIdx) {
  ...
  document.getElementById('vo-team-title').textContent = `"${grp.name}" 하위 팀 맵핑`;
...
function voRenderTree(budgetType, existIds, filter = '') {
  ...
  document.getElementById('vo-tree-container').innerHTML = html || '<div style="text-align:center;color:#9CA3AF;padding:40px;font-size:13px">검색 결과가 없습니다</div>';
}
...
function voFilterTree() {
  ...
  _voSelectedTeams.forEach(id => { const cb = document.querySelector(`#vo-tree-container input[value="${id}"]`); if (cb) cb.checked = true; });
}

function voToggleTeam(cb) {
  cb.checked ? _voSelectedTeams.add(cb.value) : _voSelectedTeams.delete(cb.value);
  document.getElementById('vo-sel-count').textContent = `${_voSelectedTeams.size}개 매핑됨`;
  document.getElementById('vo-sel-count').style.color = _voSelectedTeams.size > 0 ? '#1D4ED8' : '#111827';
...
function voConfirmAddTeams() {
  ...
  document.querySelectorAll('#vo-tree-container input:checked').forEach(cb => {
    if (_voSelectedTeams.has(cb.value)) {
      grp.teams.push({ id: cb.value, name: cb.dataset.name, budget: { allocated: 0, deducted: 0, holding: 0 } });
    }
  });
...
function voCloseModal(id) { document.getElementById(id).style.display = 'none'; }
